...
echo "<h2>Epidermal</h2>";
$sql = "SELECT `code`,`description` FROM `rast` WHERE `type` LIKE 'E' ORDER BY `rast`.`description` ASC";
...
echo "<hr/><h2>Insect</h2>";
$sql = "SELECT `code`,`description` FROM `rast` WHERE `type` LIKE 'I' ORDER BY `rast`.`description` ASC";
$results = mysqli_query($dbc,$sql);
while (list($code,$description) = @mysqli_fetch_array($results, MYSQLI_NUM)) {
  echo "<div class=\"chks\"><div class=\"e\"><input id=\"$code-1\" name=\"$code-1\" class=\"ckbx\" type=\"checkbox\" /></div><div class=\"g4\"><input id=\"$code-3\" name=\"$code-3\" class=\"ckbx\" type=\"checkbox\" /></div><div class=\"g\"><input id=\"$code-2\" name=\"$code-2\" class=\"ckbx\" type=\"checkbox\" /></div> $description</div>\n";
}
echo "<hr/><h2>Dust</h2>";
$sql = "SELECT `code`,`description` FROM `rast` WHERE `type` LIKE 'D' ORDER BY `rast`.`description` ASC";
$results = mysqli_query($dbc,$sql);
while (list($code,$description) = @mysqli_fetch_array($results, MYSQLI_NUM)) {
  echo "<div class=\"chks\"><div class=\"e\"><input id=\"$code-1\" name=\"$code-1\" class=\"ckbx\" type=\"checkbox\" /></div><div class=\"g4\"><input id=\"$code-3\" name=\"$code-3\" class=\"ckbx\" type=\"checkbox\" /></div><div class=\"g\"><input id=\"$code-2\" name=\"$code-2\" class=\"ckbx\" type=\"checkbox\" /></div> $description</div>\n";
}
...
<input type="hidden" name="grp" value="epidermal"/>
</form>
