...
if ($sub == 0){
  ...
  if ((hash('ripemd320',$passcode) == '6732f3c024fe3c3c1ccd7dbe5d0fa7d4a53c516f800707bb86a9017c1e1646d9d4a2d7adc56a505d') ){ //&& (substr($ip,0,11)  == '128.30.52.7' || $ip == '99.3.150.55' || $ip == '173.162.23.21' || $ip == '70.197.160.30')){
    $admin = true;
 //   $clear = '<div id="sc"><form action="entertest.php" method="post"><input type="hidden" name="id" value="' . $SAVE_ID . '"><input type="hidden" name="sub" value="9"><button id="clear">Reset Diet Icons</button></form></div>';  
  }
 ...
 //   $clear = '<div id="sc"><form action="entertest.php" method="post"><input type="hidden" name="id" value="' . $SAVE_ID . '"><input type="hidden" name="sub" value="9"><button id="clear">Reset Diet Icons</button></form></div>';  
      ...
      $clear = '';//<div id="sc"><form action="enter.php" method="post"><input type="hidden" name="id" value="888887"><input type="hidden" name="sub" value="9"><button id="clear">Reset Diet Icons</button></form></div>';  
  ...
  if (strlen($error) == 0 && $rows == 1) {
    ...
    if ($cli[12] == $hash || $admin === true || ($client == 888887  && ($passcode = 'allermetrix' || $passcode = 'allermetrics'))) {
      $pass = true;
...
$history = '<form action="requestFormtest.php"><input type="hidden" name="id" value="$client" /><input id="fh" type="submit" value="Patient Inhalant &amp; Food History" /></form>';
$sql = "SELECT COUNT(*) FROM `history` WHERE `client`=$client LIMIT 1 ";
$result = mysqli_query($link,$sql);
list($cnt) = mysqli_fetch_array($result, MYSQLI_NUM);
if($cnt == 0){$history = '';}
//<div id="x">X</div> red x diet no foods icon
...
.q5{background-image: linear-gradient(to bottom, #96deda  0%, #44aba5  100%);}/* qns */ 
...
.o5{color:#fff;background-image: linear-gradient(to bottom, rgba(255,0,0,1) 0%, rgba(168,0,0,1) 100%);}/* hold */
.c5{cursor:pointer;font:700 1.1em Arial,sans-serif;width:240px;letter-spacing:1px;}
.o5{font:700 1.1em Arial,sans-serif;letter-spacing:1px;}
.c6{background:#000;padding-left:6px;}
.c6{width:100%}
.h6{width:49px;padding:0 2px 0 2px;}
.h7{width:43px;padding:0 2px 0 2px;}
.q5{color:#f00;font:700 1.1em Arial,sans-serif;letter-spacing:2px;}
...
$qns = '';
$hold = '';
//if($statusCnt['Q'] > 0){$qns = ", $statusCnt['Q'] QNS Patients";}
//if($statusCnt['H'] > 0){$hold = ", $statusCnt['H'] Patients on Hold";}
$sql = "SELECT `Client`, `Patient`, `Date`, `Status`, `Link`, `ClientID`, `Last`, `First`,`Attributes`, `DoB`,`Done` FROM `Patient` WHERE $where $sort";
$results = mysqli_query($link,$sql);
$error = mysqli_error($link);
//$cutoff = time() - 15500000; // Add On 6 month cuttoff
$rows = mysqli_num_rows($results);
$error .= mysqli_error($link);
...
//	$addon = '';
//	$classao = 'nao';
//	if(strtotime($collection) > $cutoff && $volume > .5){$addon = 'Add On ' . $volume . 'ml';$classao='ao';}elseif($volume > .5){$addon = $volume;}
    echo <<<EOT
<tr>
...
EOT;
// add on 8/6/2022 <td><form action="/login/" method="post"><input type="hidden" name="client" value="$Client" /><input type="hidden" name="sub" value="55" /><input type="hidden" name="accession" value="$accession" /><button class="$classao">$addon</button></form></td>
  }
  echo '</table>';
...
echo <<<EOT
<span class="sm">$t $idcnt</span></div></div>
<script>
//<![CDATA[
const flip = [];
flip[true] = false;
