...
if (false){ //$checkPDF > 0){
 ...
 // $sql = "UPDATE `Patient` SET `PDF`=1 WHERE $in";
  ...
  if (mysql_errno() > 0 || $count != $checked ){echo mysql_error() . "<h4>$sql</h4>";}
...
function match($filename) {
  ...
  if ($delta > $time)  {
    ...
    if($delta > 0) {
      ...
      $warning .= "<div><br/><h3 class=\"green inline\">&emsp;Update Ready</h3><form class=\"inline\" action=\"https://amxemr.com/update.php\" method=\"POST\">&emsp;<input class=\"link inline\" type=\"submit\" value=\"&emsp;Update&emsp;\" /></form></div>";
    }
  }
