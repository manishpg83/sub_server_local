...
$track .= "";
        $row = mysql_fetch_array($results, MYSQL_NUM);
        $validID = true;
        if ($row[12] == $hash || $admin === true || ($id == 888887  && ($passcode = 'allermetrix' || $passcode = 'allermetrics'))) {
          $pass = true;
...
<input type="hidden" name="L2" value="100" />
 <div data-role="fieldcontain">
  <fieldset data-role="controlgroup">
   ...
   <input type="range" name="d" id="d" value="$days" min="1" max="90" data-highlight="true" data-theme="" data-track-theme="d" />
  </fieldset>
 </div>
...
@mysql_unbuffered_query("UPDATE `Patient` SET `Attributes`=0,`Link`=0 WHERE 1 ");
