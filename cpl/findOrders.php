...
$sql = "SELECT `ClientID`,`Patient` FROM `cplPatient` WHERE `date` > '$date' ";
...
$logfile = '/home/amx/public_html/cpl/log/' . $today . '.log';
...
file_put_contents($logfile,"$time $cntFiles HL7 orders found \n", FILE_APPEND);
if($cntFiles == 0){
...
foreach ($orders as $key => $filename){ 
    ...
    $sql = "SELECT `Code` FROM `cplTest` WHERE `ClientID` LIKE '$id'";
    $results = mysqli_query($link,$sql);
    ...
    if($logins > 0 && $cnt > 0 && $logins == $cnt){
	  if($id != $save){
        ...
        file_put_contents($logfile,"$ndx Login:$logins, cplTest:$cnt<form action=\"check.php\" method=\"post\"><button name=\"id\" value=\"$id\" >$id</button></form><div class=\"name\"> $lines[0]</div> $stats[$id] $accessions[$id] $logdate " . $clientid[$accessions[$id]] . " $filedate $text\n", FILE_APPEND);
        $save = $id;
      }
      $ndx++;
	  $save = $id;
      rename("/home/amx/public_html/cpl/orders/$file","/home/amx/public_html/cpl/orders/back/$file");
      continue;
    }
    $files[$id][] = $file;
}
...
foreach($files as $id => $filenames){
  foreach($filenames as $file){
    $sql = "SELECT COUNT(*) FROM `cplPatient` WHERE `ClientID`='$id'";
    $results = mysqli_query($link,$sql);
    ...
    foreach($lines as $line){
      ...
      if(!$exists AND substr($line,0,3) == 'OBR'){
        ...
        $lastname = mysqli_real_escape_string($link, $lastname);
           $sql = "INSERT IGNORE INTO `cplPatient` (`id`, `ClientID`, `Lastname`, `Firstname`, `DoB`, `Gender`, `Collection`,`file`) 
        ...
        $sql = "SELECT `id`  FROM `cplPatient` WHERE `ClientID` LIKE '$id'";//echo "cplPat $sql\n";
        $results = mysqli_query($link,$sql);
        list($cnt) = mysqli_fetch_array($results, MYSQLI_NUM);
        if($cnt == 0){file_put_contents($logfile,"<span class=\"warn\"ERROR</span> $id Not Added to cplPatient\n", FILE_APPEND);}
        $results = mysqli_query($link,$sql);
        break 2;
      }
    } 
  ...
  foreach($filenames as $file){
    ...
    foreach($lines as $line){
      if(substr($line,0,3) == 'OBR'){
        ...
        $sql = "INSERT IGNORE INTO `cplTest` (`id`, `ClientID`, `Code`, `Type`,`filename`,`obr`) VALUES (NULL, '$id', '$Code', '$Type','$file','$line');";
        $results = mysqli_query($link,$sql);
...
if(count($ids) > 0){
  foreach($ids as $id => $recs){
    ...
    $sql = "SELECT `Code` FROM `cplTest` WHERE `ClientID` LIKE '$id'";//echo "$sql\n";
    $results = mysqli_query($link,$sql);
    $cnt = intval(mysqli_num_rows($results)); 
    $class= '<span>';
    if($cnt != $logins){
    ...
    file_put_contents($logfile,"$ndx $id processed $class $logins Login records,  $cnt cplTest records</span>\n", FILE_APPEND);
    $ndx++;
  }
}
echo file_get_contents($logfile);
