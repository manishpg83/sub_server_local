...
$patients = array();
$sql = "SELECT `accession` FROM `AllergyTests` WHERE `Code` LIKE 'F360' AND `Type`=1 AND `Score` NOT LIKE '0' AND `Score` NOT LIKE '0/1' AND `Score` NOT LIKE '1'";
...
foreach($patients as $accession){$accession = intval($accession);
  ...
  $panel = 0;
  $total = 0;
  $sql = "SELECT `Code`,`Type`,`Score`,`Concentration`, Rast.description FROM `AllergyTests` JOIN Rast USING(`Code`)WHERE  `accession` = $accession;    ";
  $results = mysqli_query($link,$sql);
  ...
  while (list($Code, $Type,$Score,$Concentration,$description) = mysqli_fetch_array($results, MYSQLI_NUM)){
    ...
    if (in_array($Code, $codes)) {
	  ...
	  $Conc = ' ' . $Concentration;
	  $Conc = substr($Conc,-5);
	  $allergens[] = "Class: $score $plus$pplus $types[$Type] $Code $Conc $description";
...
exit;
$sql = "SELECT `rec`,`Code` FROM `AllergyTests` WHERE `rec`>1809385";
$results = mysqli_query($link,$sql);
while (list($rec, $Code) = mysqli_fetch_array($results, MYSQLI_NUM)){
if(mysqli_errno($link)){echo mysqli_error($link);echo "<br>$sql<br>";exit;}
  $Group = substr($Code,0,1);
  $sql = "UPDATE `AllergyTests` SET `Group`='$Group' WHERE `rec` = $rec";
  $result = mysqli_query($link,$sql);
  if(mysqli_errno($link)){echo mysqli_error($link);echo "<br>$sql<br>";exit;}
}
exit;
$handle = fopen("test.csv", "r");
while (($data = fgetcsv($handle, 128, "\t")) !== FALSE) {//var_export($data);exit;
  list(,$accession,,$Code, $Type,$range,$Concentration, $Score,$Status) = $data;
  if($Status != 'C'){continue;}
  $sql = "INSERT INTO `AllergyTests` (`rec`,`accession`, `Code`, `Type`, `Concentration`, `Score`) VALUES (NULL,$accession, '$Code', $Type, $Concentration, '$Score');";
  $results = mysqli_query($link,$sql);
  if(mysqli_errno($link)){echo mysqli_error($link);}
//  $ndx++;
//  echo "a$accession, c$Code, t$Type, r$range, c$Concentration, s$Score, s$Status<br>";
//if($ndx > 55){exit;}
}
//"Rec No"	"Tm:invoice"	"Tm:order"	"Tm:code"	"Tm:type"	"Tm:range"	"Tm:count"	"Tm:score"	"Tm:status"	"Tm:modifier"	"Tm:fee"	"Tm:date"
//1,371,362
?>