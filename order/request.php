...
#asteriskFirst,#asteriskLast,#asteriskDOB{display:none;color:#f00;}
</style></head><body>
...
$option = '';
$jsnpi = 'const npi = {'; 
$phy = "<div id=\"hcp\"><div class=\"label\">Healthcare Provider:</div> <input id=\"physican\" type=\"text\" name=\"physican\" value=\"$physician\"  autocomplete=\"off\"  oninput=\"getlength();\" />";

if($cnt > 0){
   $phy = "<div class=\"row\"><div class=\"label\">Healthcare Provider:</div> <input id=\"physican\" type=\"text\" name=\"physican\" value=\"$physician\"  list=\"phy\"placeholder=\"Click for List or Type Name\" autocomplete=\"off\" oninput=\"getnpi();\"/><span id=\"asteriskPhy\" class=\"required\"> &#x2731;&#x2731;</span></div><datalist id=\"phy\" >";
...
<div class="radio"><input type="radio"  name="gender" value="2" $checked[2] />&thinsp;Female</div></div>
...
function getlength(num){
	console.log('phy: ' + phy.value.length);
...
function chkAll(){
  ...
  console.log('chkAll: initials:' + initials.value + '|first: ' + first.value.length + '| last: ' + last.value.length + '|hcp: ' + hcp.style.display + '|age: ' + age + '|sub: ' + enablesub.style.display + '|missing: ' + missing);
  chkMissing();
}
function chkMissing(){ 
  console.log('chkMissing: ' + (missing & 6));
