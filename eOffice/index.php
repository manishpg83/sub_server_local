...
//echo "\n$rec, $sub, $client, $action\n";

if($client > 99999){
  $sql = "SELECT `Number`, `Name` FROM `Client` WHERE `Number` = $client LIMIT 1";
  $results = mysqli_query($link,$sql);
  list($client) =  mysqli_fetch_array($results, MYSQLI_NUM);
}
...
if($sub == 30){include('/home/amx/public_html/eOffice/dashboard.php');}
...
if($sub == 11){ 
  ...
  $passcode = substr('000' . $code,-4);
  if($code == 0 ||  $code > 9999) {include('/home/amx/public_html/eOffice/signin.php');exit;}
  $sql = "SELECT `Number`,`name` FROM `Client` WHERE `passcode` = $passcode LIMIT 1";
  $results = mysqli_query($link,$sql);
  list($client,$clientname) =  mysqli_fetch_array($results, MYSQLI_NUM);
  if(intval($client) > 99999 && $client < 1000000){
    include('/home/amx/public_html/eOffice/profile.php');
    exit;
  }
  include('/home/amx/public_html/eOffice/signin.php');exit;
}
...
if($sub == 13){ // coming from history in, going to foods
  $posted = array();
  ...
  $history = mysqli_real_escape_string($link,$jsn);
  $sql = "UPDATE `history` SET `history`='$history' WHERE `id` = '$rec' ";
  mysqli_query($link,$sql);
  if(mysqli_errno($link) > 0){$err .= "$jsn \n $sql \n sql_error=" .  mysqli_error($link);}
  file_put_contents('history.log',"rec=$rec $time $err\n",FILE_APPEND);
  
  include('/home/amx/public_html/eOffice/foodSelection.php');
  exit;

}
if($sub == 14){ // coming from foods going to dashboard
 foreach($_POST as $code => $v){
  ...
  if(mysqli_errno($link) > 0){
	  $err = "$sql\n" . mysqli_error($link);
	  file_put_contents('history.log',"rec=$rec $time\n $err\n",FILE_APPEND);
}
  include('/home/amx/public_html/eOffice/dashboard.php');
  exit;
}
...
//if($action = 1){$sub = 4;include('/home/amx/public_html/eOffice/index.php');exit;}
