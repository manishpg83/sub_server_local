...
.plus     { vertical-align: text-top;margin:0;display:inline-block;width:12px;height:12px;border:0;border:1px solid #000;background-image: url('data:image/gif;base64,R0lGODlhDAAMAJH/AOQAE////wAxAGUAdywAAAAADAAMAAACFISPqQsRbBxsT7l7EcZLQs+A05gUADs=');}
.minus    { vertical-align: text-top;margin:0;display:inline-block;width:12px;height:12px;border:0;border:1px solid #000;background-image: url('data:image/gif;base64,R0lGODlhDAAMAJH/AABuAP///wAxAGUAdywAAAAADAAMAAACDoSPqcvtG6KMaM6Hs8YFADs=');}
.checkmark{ vertical-align: text-top;margin:0;display:inline-block;width:12px;height:12px;border:0;border:1px solid #000;background-image: url('data:image/gif;base64,R0lGODlhDAAMAJEAAPZIAPv+/AAxAGUAdywAAAAADAAMAAACFISPqZsRDNxb8jh78atNTsVB4igWADs=');}
.up{ vertical-align: text-top;margin:0;display:inline-block;width:12px;height:12px;border:1px solid #b80;background-image: url('data:image/gif;base64,R0lGODlhDAAMAJEAAPhSAv769wAxAGUAdywAAAAADAAMAAACF4SPqQsR3NxTUiYXs8GaI7+A1MSI0FEAADs=');}
.blank    {vertical-align: text-top;margin:0;display:inline-block;width:12px;height:12px;border:1px solid #000;background:#eee;}
...
//	   echo $client.' - ' . $patient . '<br><br><pre>LINKS<br/>' . var_export($links,true)  . '</pre><br/>';
    ...
    if ($cnt == 1){
	 ...
	 mysqli_query($dbc,"DELETE FROM `Link` WHERE `next` = $links[0]"); 
	}
    elseif ($cnt > 1){
	  $results = mysqli_query($dbc,"SELECT MAX(`next`) FROM `Link` WHERE 1") ;
	  $row = mysqli_fetch_array($results, MYSQLI_NUM);
	  $next = $row[0];
	  $next++;
	  mysqli_query($dbc,"INSERT INTO `amx_portal`.`Link` (`next`) VALUES (NULL);");
	  echo "<h2>$cnt $next</h2>";
     foreach ($links as $v){
        $results = mysqli_query($dbc,"SELECT `Link` FROM `Patient` WHERE `Patient` = $v LIMIT 1");
	    $rows = mysqli_num_rows($results); 
	    if ($rows == 1 ){
		  $row = mysqli_fetch_array($results, MYSQLI_NUM);
		  if ($row[0] > 0){
	        mysqli_query($dbc,"UPDATE `Patient` SET `Link`=0 WHERE `Link`= $row[0]");
	      } 
        }

	  }
      foreach ($links as $v){
		@mysqli_query($dbc,"UPDATE `Patient` SET `Link`=$next WHERE `patient`= $v");
      }
    } 
}
...
$ndx++;$t[$ndx] = microtime(true);
$ndx++;$t[$ndx] = microtime(true);
mysqli_query($dbc,"CREATE TABLE Foods200223 LIKE `Allergens`");
$ndx++;$last = $ndx-1;$t[$ndx] = microtime(true);$et = number_format($t[$ndx] - $t[$last],5); echo "<p>$ndx) CREATE LIKE:  $et</p>";
mysqli_query($dbc,"SELECT * INTO OUTFILE '/tmp/table1.txt' FROM `Allergens`");
$ndx++;$last = $ndx-1;$t[$ndx] = microtime(true);$et = number_format($t[$ndx] - $t[$last],5); echo "<p>$ndx) SAVE DATA:  $et</p>";
mysqli_query($dbc,"ALTER TABLE Foods200223 DISABLE KEYS");
$ndx++;$last = $ndx-1;$t[$ndx] = microtime(true);$et = number_format($t[$ndx] - $t[$last],5); echo "<p>$ndx) DISABLE KEYS: $et</p>";
mysqli_query($dbc,"LOAD DATA INFILE '/tmp/table1.txt' INTO TABLE Foods200223");
$ndx++;$last = $ndx-1;$t[$ndx] = microtime(true);$et = number_format($t[$ndx] - $t[$last],5); echo "<p>$ndx) IMPORT DATA:  $et</p>";
mysqli_query($dbc,"ALTER TABLE Foods200223 ENABLE KEYS");
...
if (strlen($patient) == 6){
  ...
  if (strlen($error) == 0 AND $rows == 1){
	...
	$patID = $pat[3];
    $clientErr = $client;
	$name = '<h2>' . htmlspecialchars($pat[2]) . ', ' . htmlspecialchars($pat[1]);
	...
	if ($pat[5] > 0){
	  ...
	  $rows = @mysqli_num_rows($results);
	  if ($rows > 1){
	    while ($pats = mysqli_fetch_array($results, MYSQLI_NUM)){
	      $patients[] = $pats[0];
		}
	  }
	  else{
	  ...
	  $name =  '<form enctype="application/x-www-form-urlencoded" action="link.php" method="post"><div><fieldset class="widthWide"><legend>Patient</legend><input type="hidden" name="sub" value="1" /><input type="hidden" name="lnk" value="' . $pat[5] . '" /><input type="hidden" name="p" value="' . $patient .  '" />' . $name . '<br/><button class="login">Unlink (' . $links . ')</button></fieldset></div></form>';
	}
	else{
...
if (strlen($client) == 6 AND strlen($patient) == 6){
  ...
  @mysqli_query($dbc,"SELECT 1 FROM $foodsTable LIMIT 1 ");
  if (mysqli_errno($dbc) == 1146){
    mysqli_query($dbc,"CREATE TABLE $foodsTable LIKE `Allergens`");
    mysqli_query($dbc,"ALTER TABLE $foodsTable DISABLE KEYS");
    mysqli_query($dbc,"INSERT INTO $foodsTable SELECT * FROM `Foods` ORDER BY `alpha` ASC,`Description` ASC");
    mysqli_query($dbc,"ALTER TABLE $foodsTable ENABLE KEYS");
  }
...
if ($sub == 2){
  ...
  @mysqli_query($dbc,"DELETE FROM `Exceptions` WHERE `Patient`= $patient");
  foreach ($_POST as $k => $v){
    if (substr($k,0,1) == 'e'){
      $code = substr($k,2);
...
$iUp = $iUps[$strike];
  $getIcon = array( // getIcon[exc][poz/neg/untested]
...
$setInc = array(  
      0=> array(0=>0,1=>0,2=>0),
      1=> array(0=>0,1=>1,2=>1));
 $time['create icons'] = number_format((microtime(true) - $startTime),3);                                                                                                        //  $tdx++;$t[$tdx] = number_format((microtime(true) - $startTime),3);$last = $tdx-1;$et = $t[tdx] - $t[$last]; echo "<p>Start Data time: $t[$tdx]: $et</p>";
  $foods = array();
//                 0       1 v0         2 v6   3 v7  4 v8    5 v9
  ...
  $results = mysqli_query($dbc,$sql);
  while ($fd = mysqli_fetch_array($results, MYSQLI_NUM)){
    $code2id[$fd[0]][] = $fd[5];
    if (isset($foods[$fd[5]])){continue;}
	$grp = intval($fd[2]);
	$day = intval($fd[3]);
    $foods[$fd[5]] = array($fd[1],0,0,0,0,0,$grp,$day,$fd[4],$fd[0],'','','');
	if ($fd[4] == 0 ||($fd[3] & 8 || $fd[3] == 0) ){continue;}
	$groups [$grp][$fd[1]] = $fd[5];
  }
  $time['build food table'] = number_format((microtime(true) - $startTime),3);
 /*
  $fp = fopen("/home/amx/public_html/mugwort.txt","w");
 // $fp = fopen("/home/amx/public_html/sagebrush.txt","w");
//  $fp = fopen("/home/amx/public_html/ragweed.txt","w");

$code = array('F049','F288','F031','F085','F247','F208','F134','F351','F087','F033','F013','F210','F129','F044','F329');
//$code = array('F288','F031','F085','F208','F134','F351','F033','F013','F210','F129','F044','F329');
//$code = array('F092','F288','F244','F208','F134','F351','F087','F033','F013','F210','F129','F044','F316','F329');
 ...
 /* 
$codes[] = array('T003','T146','T008','T107','T108','T100','T101','T153',T149); 
$codes[] = array('T140');
$codes[] = array('W006'); 
$codes[] = array('W111'); 
$codes[] = array('W001'); 
...
$grass = 5;
$latex = 6; 
$cx[$trees] = array('F020','F049','F138','F096','F092','F343','F288','F018','F031','F085','F100','F341','43','F259','F017','F153','F208','F134','F351','F132','F033','F283','F112','F144','F095','F013','F094','F263','F313','F314','F150','F210','F128','32','F035','F115','F129','F014','F044','F025','F256','F317','F329','F004');
$cx[$sycamore] = array('F049','F288','F142','43','F017','F208','F215','F134','F008','F351','F033','F095','F013','F210','32','F129','F044','F329');
$cx[$mugwort] = array('F049','F288','F031','F085','43','F247','F208','F134','F351','F087','F033','F013','F210','32','F129','F044','F329');
$cx[$sagebrush] = array('F288','F031','F085','43','F208','F134','F351','F033','F013','F210','32','F129','F044','F329');
$cx[$ragweed] = array('F092','F288','43','F244','F208','F134','F351','F087','F033','F013','F210','32','F129','F044','F316','F329');
$cx[$grass] = array('F356','F300','F301','F302','F303','F356','F142','F015','F288','F007','F008','F304','F004','FZ11','F009','F350','32','F153','F013','F012','F355','F210','32','F129','F014','F044','F025','F329','FOO4');
$cx[$latex] = array('F049','F096','F092','F031','F085','F103','F153','F087','F305','F329','F127','F035','F025');

 
 foreach ($cx as $group => $v ){
    $dx = array();
    foreach ($v as $code ){
	  $id = $code2id[$code][0];
	  $description = $foods[$id][0];
	  $dx[$description] = $id;
	  $c[$id] = $code;
    }
	ksort($dx);
	$data = "\$cx[$group] = array(";
	$formated = $cxGroup[$group] . '(';
	$codes = $cxGroup[$group] . '(';
    foreach ($dx as $id ){
	  $data .= "$id,";
	  $formated .= sprintf("'%04d',",$id);
	  $codes .= "'$c[$id]',";
	}
...
foreach ($patients as $p){
  ...
  while ($row = @mysqli_fetch_array($results, MYSQLI_NUM)){
	...
	$ndx = $row[1]+9;
	 $foods[441][14] = $foods[441][5] ; $pos=14;
	foreach ($code2id[$row[0]] as $k => $id){
	  $ndx = $row[1]+9;
	  $foods[$id][$ndx] = $row[2];
      //& $sub;
      $foods[$id][$row[1]] = $getPoz[$cutoff][$row[2]];
...
//	if ($foods[$id][5] == 2){$positives[$row[0]] = array($foods[$id][0] ,$foods[$id][10] , $foods[$id][11] , $foods[$id][12]);}
	}
	mysqli_free_result($results);
}	
...
if ($sub == 1){
  $results = mysqli_query($dbc,"SELECT `id`,`value` FROM `Exceptions` WHERE `Patient` = $patient ");
  while ($excpt = mysqli_fetch_array($results, MYSQLI_NUM)){
    $ix[282][] = 'excpt b4:' . $foods[$excpt[0]][4];
    $foods[$excpt[0]][4] |= 4;
    $foods[$row[0]][4] = 1;
    $ix[282][] = 'excpt aft:' . $foods[$excpt[0]][4];
  }
  mysqli_free_result($results);
}
$time['combine patient & food'] = number_format((microtime(true) - $startTime),3);
  if ($foods[290][5] == 2){
    $component = array(282, 283, 284, 285, 286, 287, 288, 289,297 );
  ...
  if ($foods[298][5] == 2 || $foods[299][5] == 2 || $foods[300][5] == 2 || $foods[301][5] == 2 || $foods[302][5] == 2){
    $component = array(298,299,300,301,302);
	foreach ($component as $v){
      $foods[$v][4] |= $priority[$foods[$v][5]];
	}
  }
  ...
  if ($foods[131][5] == 2 || $foods[132][5] == 2 || $foods[133][5] == 2  || $foods[134][5] == 2 || $foods[135][5] == 2){
    $component = array(131,132,133,134,135);
	foreach ($component as $v){
      $foods[$v][4] |= $priority[$foods[$v][5]];
	}
  }
...
<button class="login">Get New Patient</button>  
</fieldset></form>
EOT;
}
echo <<<EOT
...
$name
<form method="post" enctype="application/x-www-form-urlencoded" action="d.php">
...
$gc[$v[0]]=array($getChk[$v[4]][$v[5]][0],$v[4],$v[5]);
  if ($getChk[$v[4]][$v[5]][0]===true|| $v[7] == 9){continue;}  // (substr($v[9],0,1) !='F' ||
  $chk++;
  $ic[0] = $v[9];
  $ic[1] = $k;
...
if (count($pozChk) > 0){
echo "<fieldset class=\"pozBox\"><h2>Positive Foods</h2><p>Check to Include in Diet and Grocery List</p><fieldset><button class=\"login\">Update</button> </fieldset>";
ksort($pozChk);
  foreach ($pozChk as $k => $v){
    echo $v;
  }
  echo '<br/><button class="login">Update</button> </fieldset>';
  unset($k,$v,$pozChk);
}
else{
...
<legend>Positive Score Cutoff (current=$cutoff)</legend> 
...
flush();
ob_end_flush();
mysqli_query($dbc,"UPDATE `Patient` SET `Attributes` = $linked  WHERE `Patient`= $patient ");
}
}
  if (!isset($sub) || $sub == 0){
...
if ($sub == 2 && count($flip) > 0){
  mysqli_query($dbc,"DELETE FROM `Exceptions` WHERE `Patient` = $patient");
  foreach ($flip as $k=>$v){
    mysqli_query($dbc,"INSERT INTO `amx_portal`.`Exceptions` (`Patient`, `id`, `Offset`, `value`) VALUES ($patient, '$k', 4, '4')");
  }
}
