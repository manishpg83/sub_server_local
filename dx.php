...
if (strlen($client) == 6 AND strlen($patient) == 6){
 ...
 if (mysqli_errno($dbc) == 1146){
  ...
  mysqli_query($dbc,"INSERT INTO $foodsTable SELECT * FROM `Foods888887` ORDER BY `alpha` ASC,`Description` ASC");
 ...
 $results = mysqli_query($dbc,$sql);
 while ($fd = mysqli_fetch_array($results, MYSQLI_NUM)){list($Code,$Description,$Alpha,$Day,$Group,$Id ) = $fd;
  $code2id[$fd[0]][] = $fd[5];
