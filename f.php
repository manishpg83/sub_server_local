...
$patientname = trim(urldecode($_POST['patientname']));
...
#page{width:480px;margin:0 auto;padding:0;border:0;}
...
.btn,.link,.go{width:100%;padding: .25em;margin:.7em 0 0 0;border-radius: 3px 3px 3px 3px;font: 700 1em Arial,Helvetica,Calibri,sans-serif;overflow: visible;}
...
.hide,#bs{background:#144;display:block;font-weight:400;margin:0 0 .5em;padding:.2em;text-align:center;}
...
.dchk,.chk,.component{background:#2985EA;text-align:left;position:relative;}
...
.fchk{position:relative;text-align:left;background:#2985EA;padding:0;margin:.4em 0 ;color:#fff;display:inline-block;font:700 1em Arial,sans-serif;}
.pchk{text-align:left;background:#2985EA;width:80%;padding:0;margin:.4em 0 ;color:#fff;display:inline-block;font:700 1em Arial,sans-serif;;}
.panel{width:14em;padding:0 .1em 0 .1em;display:inline-block;font-weight:700;}
.desc{font-size:.9em;width:12em;padding:0 .1em 0 .1em;display:inline-block;font-weight:700;}
#col1{background:#144;color:#ff0;font-weight:700;padding:4em;}
.col{background:#144;width:480px;margin:0;border:0;}
...
.ige,.pige{background:#ff0;color:#000;}
.igg4,.pigg4{background:#0ff;color:#000;}
...
</style></head><body><div id="page"><p>dob: $dob<br/>date: $date<br/>strdob: $strdob<br/>len: $len<br/>gender: $gender</p>
EOT;
...
$client
<br/>Name: <input type="text" name="patientname" value="$patientname" />
<br/>DoB: <input type="text" name="dob" value="$strdob" placeholder="m/d/yyyy" />
<div id="d001" class="dchk"><input id="c001" type="radio" class="chk" name="gender" value="1" onclick="radio('001','002')" $checked[1]/>&#x2002;Male </div>
<div id="d002" class="dchk"><input id="c002" type="radio" class="chk" name="gender" value="2" onclick="radio('002','001')" $checked[2]/>&#x2002;Female</div>


<p>&#x2003;<br/>Enter Your Name and Date of Birth<br/>
Click Begin then,<br/>
Select the foods you may be allergic to.
</p>
...
</p></div></div><div id="post"></div>
EOT;
}
elseif($sub == 1){
  echo'<div id="col"><form action="f.php" method="post"><div id="post">';
...
foreach($_POST as $k => $v){
  if(substr($k,0,1) == 'F'){
    ...
    $showE[$k] = intval($rank[1][$k]);
    $showG4[$k] = intval($rank[3][$k]);
  }
}
$desc = $rank[0];
$food[0] = $showE;
...
echo <<<EOT
<pre>$show</pre>
<button id="summary" class="go" type="button" onclick ="summary('s14')">Summary</button><div id="s14" class="hide">
<h3>Patient Summary</h3>
<button class="link" type="button" onclick ="panels('s16')">Patient Foods</button>
<div id="s16" class="hide">
  <div class="row">
...
echo <<<EOT
</div><button class="link" type="button" onclick ="panels('s15')">Panels</button>
<div id="s15" class="xhide"><h3>Client Panels</h3>
EOT;
$types = array('','IgE','IgG','IgG4');
$panels = array();
$panels['770']= array('Texas Panel','',40,0);
$panels['771']= array('Wellness Panel','no',42,0) ;
$panels['772']= array('Food Panel 30','no',30,30) ;
...
$panels = array();
$panels['910'] = 'Total IgE';
$panels['911'] = 'High Dil.Total IgE';
$panels['920'] = 'Hymenoptera (5-IgE)';
$panels['921'] = 'Ped (T-IgE/18IgE/10 IgG4)';
$panels['922'] = 'MEX Z-1';
$panels['923'] = 'MEX Z-2';
$panels['924'] = 'MEX Z-3';
$panels['925'] = 'MEX Z-4';
...
$panels['928'] = 'MEX Z-7';
$panels['929'] = 'MEX Z-8';
$panels['950'] = 'Aller food ID (IgE)';
$panels['951'] = 'Aller food ID (IgG)';
$panels['952'] = 'Aller food ID (IgG4)';
$panels['953'] = 'Aller molds(IgG)';
$panels['900'] = 'Aller-ID TN';
$panels['921'] = 'Aller-ID FL';
$panels['922'] = 'Aller-ID GA';
$panels['923'] = 'Aller-ID AZ';
$panels['924'] = 'Aller-ID TX';
$panels['925'] = 'Aller-ID MO';
$panels['976'] = 'Food Pnl (33IgE/IgG4)';
$panels['997'] = 'Food Pnl(40IgE/IgG4)';
$panels['991'] = 'Food Pnl(28IgE/22IgG4)';
$panels['992'] = 'Total IgG';
$panels['993'] = 'Total IgA';
$panels['994'] = 'Total IgM';
$panels['995'] = 'Immuno Pnl (TIgG/TIgA/TIgM)';
$panels['991'] = 'Gastro Pnl(46IgE/45IgG4)';
$panels = array();
$ndx = 0;
$fp = fopen("/home/amx/public_html/panels.txt", "r");
while (($text= fgets($fp , 64)) !== false) {
  $ndx++;
  if($ndx > 30){break;}
  $panel = substr($text,0,8);
  ...
  $panels[$panel] =array($description,$TestTypeCount[1],$TestTypeCount[3]);
}
foreach($panels as $panel => $v){
echo <<<EOT
  <div id="d$panel" class="pchk">
  <input type="checkbox"  id="c$panel" class="chk" name="$panel" value="$kg4" onclick="ck('$panel')" />
  <div class="panel">&#x2002;$v[0]</div>
  <div class="inline" onclick="exp(p$panel)"><div class="total">X</div><div class="pige">$v[1]</div><div class="pigg4">$v[2]<</div></div></div><br/>
EOT;
  foreach($tests[$panel] as $test){
  echo <<<EOT
<p class="">$test[0] $test[1]</p>
EOT;
  }
}
echo <<<EOT
<div class="total" style="width:2.4em">TIgE</div>
<div class="pige" style="width:2.4em">IgE</div>
<div class="pigg4" style="width:2.4em">IgG4</div><br/>
<br/>
EOT;

echo '<h3>Allermetrix Panels</h3>';


foreach($panels as $num => $text){
echo <<<EOT
  <div id="d$num" class="pchk"><input type="checkbox"  id="c$num" class="chk" name="$num" value="$kg4" onclick="ck('$num')" /><div class="desc" style="width:16em">&#x2002;$text</div></div> <br/> 
EOT;
 } 
...
<hr/>
<h3>Order Individual Allergens</h3>
...
echo '<hr/><h3>Next Top 15</h3><div class="half">IgE</div><div class="half">IgG4</div>';
$ndx = 0;
...
<pre>$show</pre></div><div id="s888"></div>
...
function radio(on,off){
  d[on].style.backgroundColor=bg[c[on].checked];
  d[off].style.backgroundColor=bg[c[off].checked];
}
function ck(id){
...
var divs = document.getElementsByTagName("div");
  ...
  document.getElementById('s14').style.display = '$s14';
}
