...
if (strlen($patient) == 6){
  ...
  if (strlen($error) == 0 AND $rows == 1){
    ...
    if (strlen($error) == 0 AND $rows == 1){
      ...
      $sql = "SELECT * FROM `template` WHERE `Patient` = $patient LIMIT 1"; 
      $results = @mysqli_query($dbc,$sql);
      $error = mysqli_error($dbc);
	  $rows = @mysqli_num_rows($results);
      if (strlen($error) != 0 || $rows != 1){
        $err .= "<br/>SEL #1 PAT TPL: $error<br/>$sql<br/>";
        $sql = "SELECT * FROM `template` WHERE `Client` = 0 AND `Patient` = 0 LIMIT 1"; 
        $results = @mysqli_query($dbc,$sql);
        $error = mysqli_error($dbc);
        $rows  = @mysqli_num_rows($results);
        if (strlen($error) != 0 || $rows != 1){
          $sql = "SELECT `Client`, `Patient`, `CutOff`, `CutOffG4`, `excludeMethod`, `CrossreactivityGrocery`, `CrossreactivityDiet`, `CrossreactivityPhycian`, `CrossreactivityPatient`, `HiddenFoodPhysician`, `HiddenFoodPatient`, `ExcludeIgE`, `ExcludeIgG`, `ExcludeIgG4` FROM `template` WHERE `Client` = 999999 AND `Patient` = 0 LIMIT 1"; 
          $results = @mysqli_query($dbc,$sql);
          list($clientx, $Patient, $CutOffE, $CutOffG4, $excludeMethod, $CrossreactivityGrocery, $CrossreactivityDiet, $CrossreactivityPhycian, $CrossreactivityPatient, $HiddenFoodPhysician, $HiddenFoodPatient, $ExcludeIgE, $ExcludeIgG, $ExcludeIgG4) =  mysqli_fetch_array($results, MYSQLI_NUM);
          $sql = "INSERT INTO `amx_portal`.`template` (`Client`, `Patient`, `CutOff`, `CutOffG4`, `excludeMethod`, `CrossreactivityGrocery`, `CrossreactivityDiet`, `CrossreactivityPhycian`, `CrossreactivityPatient`, `HiddenFoodPhysician`, `HiddenFoodPatient`, `ExcludeIgE`, `ExcludeIgG`, `ExcludeIgG4`) VALUES ('$client', '0','$cutoff', '$excludeMethod', '$CrossreactivityGrocery', '$CrossreactivityDiet', '$CrossreactivityPhycian', '$CrossreactivityPatient', '$HiddenFoodPhysician', '$tpl[1]', '$ExcludeIgE', '$ExcludeIgG', '$ExcludeIgG4')";
          $results = @mysqli_query($dbc,$sql);
        }
        else{
          list($clientx, $Patient, $CutOffE, $CutOffG4, $excludeMethod, $CrossreactivityGrocery, $CrossreactivityDiet, $CrossreactivityPhycian, $CrossreactivityPatient, $HiddenFoodPhysician, $HiddenFoodPatient, $ExcludeIgE, $ExcludeIgG, $ExcludeIgG4) = mysqli_fetch_array($results, MYSQLI_BOTH);
        }
        $sql = "INSERT INTO `amx_portal`.`template` (`Client`, `Patient`, `CutOff`, `CutOffG4`, `excludeMethod`, `CrossreactivityGrocery`, `CrossreactivityDiet`, `CrossreactivityPhycian`, `CrossreactivityPatient`, `HiddenFoodPhysician`, `HiddenFoodPatient`, `ExcludeIgE`, `ExcludeIgG`, `ExcludeIgG4`) VALUES (0, $patient, '$cutoff', '$excludeMethod', '$CrossreactivityGrocery', '$CrossreactivityDiet', '$CrossreactivityPhycian', '$CrossreactivityPatient', '$HiddenFoodPhysician', '$tpl[1]', '$ExcludeIgE', '$ExcludeIgG', '$ExcludeIgG4')";
        $results = @mysqli_query($dbc,$sql);
      }
      else{
        list($clientx, $Patient, $CutOffE, $CutOffG4, $excludeMethod, $CrossreactivityGrocery, $CrossreactivityDiet, $CrossreactivityPhycian, $CrossreactivityPatient, $HiddenFoodPhysician, $HiddenFoodPatient, $ExcludeIgE, $ExcludeIgG, $ExcludeIgG4) =  mysqli_fetch_array($results, MYSQLI_NUM);
...
EOT;
  if ($sub > 1){
    $CrossreactivityGrocery  = intval($_GET['C4']);
    $CrossreactivityDiet  = intval($_GET['C5']);
    $CrossreactivityPhycian  = intval($_GET['C6']);
    $CrossreactivityPatient  = intval($_GET['C7']);
    $HiddenFoodPhysician  = intval($_GET['C8']);
    $HiddenFoodPatient  = intval($_GET['C9']);
    $ExcludeIgE  = intval($_GET['C10']);
    $ExcludeIgG  = intval($_GET['C11']);
    $ExcludeIgG4  = intval($_GET['C12']);
    $CutOffE = intval($_GET['R2']);
    $CutOffG4 = intval($_GET['R1']);
    $excludeMethod  = intval($_GET['R3']);
    

    $sql = "UPDATE `template` SET `CutOff` = $CutOffE,`CutOffG4` = $CutOffG4,`excludeMethod`  = $excludeMethod,`CrossreactivityGrocery` = $CrossreactivityGrocery,`CrossreactivityDiet` = $CrossreactivityDiet,`CrossreactivityPhycian` = $CrossreactivityPhycian,`CrossreactivityPatient` = $CrossreactivityPatient,`HiddenFoodPhysician` = $HiddenFoodPhysician,`HiddenFoodPatient` = $HiddenFoodPatient,`ExcludeIgE` = $ExcludeIgE,`ExcludeIgG` = $ExcludeIgG,`ExcludeIgG4` = $ExcludeIgG4  WHERE `Patient` = $patient";
    $results = @mysqli_query($dbc,$sql);
    $error = mysqli_error($dbc);
 // if (strlen($error) > 0){
...
$cutoffValueE = $cutOffValues[$CutOffE]; 
$cutoffValueG4 = $cutOffValues[$CutOffE]; 
$cutOffEChecked[$CutOffE] = 'checked="checked"';
$cutOffG4Checked[$CutOffG4] = 'checked="checked"';
$mode[$excludeMethod] = 'checked="checked"';
$ndx = 4;
$check = array('','checked="checked"','checked="checked"');
$checked[4] = $check[$CrossreactivityGrocery];
$checked[5] = $check[$CrossreactivityDiet];
$checked[6] = $check[$CrossreactivityPhycian];
$checked[7] = $check[$CrossreactivityPatient];
$checked[8] = $check[$HiddenFoodPhysician];
$checked[9] = $check[$HiddenFoodPatient];
$checked[10] = $check[$ExcludeIgE];
$checked[11] = $check[$ExcludeIgG];
$checked[11] = $check[$ExcludeIgG4];
$err .= "$CrossreactivityGrocery, $CrossreactivityDiet, $CrossreactivityPhycian, $CrossreactivityPatient, $HiddenFoodPhysician, $HiddenFoodPatient, $ExcludeIgE, $ExcludeIgG, $ExcludeIgG4\n";

$err .= '<br/>CHECKED<pre>' . var_export($checked,true) . '</pre><br/>';
if($ip != '70.171.10.12' ){ $err = '';}
echo <<<EOT
...
<legend>Positive Score IgE Cutoff IgE</legend>
<div id="d13" class="divNoCheck" ><input id="c13"  class="noCheck" type="radio" name="R2" value="1"  onclick="radio(13,13,15)"  $cutOffEChecked[1] />&#160;&#160;0/1&#x2003;</div><br/>
<div id="d14" class="divNoCheck" ><input id="c14"  class="noCheck" type="radio" name="R2" value="2"  onclick="radio(14,13,15)"  $cutOffEChecked[2] />&#160;&#160;1&#x2003;</div><br/>
<div id="d15" class="divNoCheck" ><input id="c15"  class="noCheck" type="radio" name="R2" value="3"  onclick="radio(15,13,15)"  $cutOffEChecked[3] />&#160;&#160;2&#x2003;</div>
<p><br/></p><p>Current Score Cutoff: $cutoffValueE</p>
</fieldset><div><br/></div><fieldset class="widthWide">
<legend>Positive Score Cutoff IgG4</legend>
<div id="d16" class="divNoCheck" ><input id="c16"  class="noCheck" type="radio" name="R1" value="1"  onclick="radio(16,16,18)"  $cutOffG4Checked[1] />&#160;&#160;0/1&#x2003;</div><br/>
<div id="d17" class="divNoCheck" ><input id="c17"  class="noCheck" type="radio" name="R1" value="2"  onclick="radio(17,16,18)"  $cutOffG4Checked[2] />&#160;&#160;1&#x2003;</div><br/>
<div id="d18" class="divNoCheck" ><input id="c18"  class="noCheck" type="radio" name="R1" value="3"  onclick="radio(18,16,18)"  $cutOffG4Checked[3] />&#160;&#160;2&#x2003;</div>
<p><br/></p><p>Current IgG4 Score Cutoff: $cutoffValueG4</p>
...
</fieldset><fieldset class="widthWide">
...
$display[0][NULL] = 'blank';
  ...
  $exclude = array(0,$ExcludeIgE,$ExcludeIgG4,$ExcludeIgG);
  ksort($_GET);
  foreach ($_GET as $k => $v){
    if (substr($k,0,1) == 'a'){
...
if ($sub > 1){
  ...
  $sql = "UPDATE `Test` SET `Attributes`= " . $attr[$ExcludeIgE] . " WHERE `Score` BETWEEN '$CutOffE' AND '6' AND `Patient` = $patient AND `Type` = 1" ;
  $results = @mysqli_query($dbc,$sql);
        if (true){ //strlen($error) > 0 ){
          $err .= "<br/>UPDATE TEST IgE ERROR: $error<br/>$sql<br/>";
        }
  $sql = "UPDATE `Test` SET `Attributes`= " . $attr[$ExcludeIgG4] . " WHERE `Score` BETWEEN '$CutOffG4' AND '6' AND  `Patient` = $patient AND `Type` = 2" ;
  $results = @mysqli_query($dbc,$sql);
        if (true){ //strlen($error) > 0 ){
          $err .= "<br/>UPDATE TEST IgG ERROR: $error<br/>SQL: $sql<br/>";
        }
  $sql = "UPDATE `Test` SET `Attributes`= " . $attr[$ExcludeIgG] . " WHERE `Score` BETWEEN '$CutOffG4' AND '6' AND  `Patient` = $patient AND `Type` = 3" ;
  $results = @mysqli_query($dbc,$sql);
        if (strlen($error) > 0 ){
...
$type = array('Added','IgE','IgG','IgG4');
  $sql = "SELECT `Code`,`Type`,`Score`,`Attributes`  FROM `Test` WHERE `Patient` =  $patient ORDER BY `Type` ASC,`Score` ASC ";
  $results = @mysqli_query($dbc,$sql);
  $error = mysqli_error($dbc);
  $rows = @mysqli_num_rows($results);
  if (strlen($error) > 0){
...
//       }
  ...
  echo "<h2>No Positives</h2><p class=\"emp\">IgE Cut Off: $cutoffValueE <br/>IgG4 Cut Off: $cutoffValueG4</p>";
}
...
//  $data[$row[1]][$row[0]] = array($score,$desc[3],$attr[4]);
  ...
  while($row = mysqli_fetch_array($results, MYSQLI_NUM)){
    ...
    $mark = $markClass[$excludeMethod][$d];
    $c = $display[$excludeMethod][$d];
