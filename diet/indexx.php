...
<form method="GET" action="https://dev.amxemr.com/diet/">
...
Exclude 10-12</legend>
<div id="d10" class="divNoCheck" >
<input id="c10"  class="noCheck" type=checkbox name="C10" value="2" $checked[10] onclick="check(10)" />
&nbsp;&nbsp;IgE</div><br/>
<div id="d11" class="divNoCheck" ><input id="c11"  class="noCheck" type=checkbox name="C11" value="2" $checked[11] onclick="check(11)" />&nbsp;&nbsp;IgG4</div><br/>
<div id="d12" class="divNoCheck" ><input id="c12"  class="noCheck" type=checkbox name="C12" value="2" $checked[12] onclick="check(12)" />&nbsp;&nbsp;IgG</div>
<p><br></p><fieldset class="widthInside"><legend>Method R3 2-3</legend>
...
<fieldset class="widthWide"><legend>Display 4-5</legend>
...
<fieldset class="widthWide"><legend>Display 8-9</legend>
...
//  $marker = array('hide','hide','minus',$markDisplay,$markDisplay);
  $exclude = array(0,$tpl[10],$tpl[11],$tpl[12]);
...
if ($sub > 1){
  ...
  $sql = "UPDATE `Test` SET `Attributes`= $tpl[10] WHERE `Score` BETWEEN '$cutoff' AND '6' AND `Patient` = $patient AND `Type` = 1" ;
  $results = @mysqli_query($dbc,$sql);
        if (true){ //strlen($error) > 0 ){
          $err .= "<br>UPDATE TEST IgE ERROR: $error<br>$sql<br>";
	    }
  $sql = "UPDATE `Test` SET `Attributes`= $tpl[11] WHERE `Score` BETWEEN '$cutoff' AND '6' AND  `Patient` = $patient AND `Type` = 2" ;
  $results = @mysqli_query($dbc,$sql);
        if (true){ //strlen($error) > 0 ){
          $err .= "<br>UPDATE TEST IgG ERROR: $error<br>SQL: $sql<br>";
	    }
  $sql = "UPDATE `Test` SET `Attributes`= $tpl[12] WHERE `Score` BETWEEN '$cutoff' AND '6' AND  `Patient` = $patient AND `Type` = 3" ;
  $results = @mysqli_query($dbc,$sql);
        if (strlen($error) > 0 ){
...
$type = array('Added','IgE','IgG','IgG4');
  ...
  while ($row = mysqli_fetch_array($results, MYSQLI_NUM)){
	...
	$class[$row[0]] = $row[4];
    if ($row[2] >= $cutoff){
	  $excludeType[$row[0]][$row[1]] = $row[4];
      if ($exclude[$row[1]][$row[2]] == 1){
        $class[$row[0]] = $row[4];
      }
      else {
        $class[$row[0]] = $row[4];
...
//	   }
         ...
         echo "<fieldset class=\"pozCheck\">$type[$k] $key <div class=\"desc\">$v2[1]</div><div class=\"score\"> $v2[0]</div><div id=\"d$chk\"  class=\"" . $excludeDivClass[$excludeType[$key][$k]] . "\" ><input id=\"c$chk\"  class=\"" . $excludeChkClass[$excludeType[$key][$k]] . "\" type=checkbox name=\"e-$key-$k\" value=\"7\" onclick=\"check('$chk')\"" . $check[$excludeType[$key][$k]] . "  />&emsp;Exclude from Positive&nbsp;</div></fieldset><br>\n";
       }
    }
  } 
}
