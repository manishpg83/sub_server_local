...
echo <<<EOT
<!DOCTYPE html><head><title>newIndex</title><style>h1{color:#f00;}</style></head><body>

<h1>No Record Submitted</h1>';
</body></html>
EOT;
exit;
}
...
//▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼
  ...
  foreach($Gfoods as $key => $code){
	...
	$sortG[] = $id;
	$g .= "$id,";
    $foods[3] .= "$id:[3,null,null,false,false,'$code','$ndx $desc[$code]','$key',false,false],";
	$gcode[] = $code; 
	$gid[] = "$id";
	$gkey[] = "$key";
  }  
  ...
  foreach($Efoods as $key => $code){
	...
	$sortE[] = $id;
	$e .= "$id,";
    $foods[1] .= "$id:[1,null,null,false,false,'$code','$desc[$code]','$key',false,false],";
	$list .= <<<EOT

<div class="row"  >
  <div id="d$id" class="echk" title="1" />
    <input type="checkbox"  id="c$id" class="chk" value="$id" name="$code" onclick="eck($id)"/>
    <div id="dc$id" class="descE" onclick="clickit($id)">&#x2002;&#x2002;$key $desc[$code]</div>
  </div>&#x2003;
EOT;
$id = $gid[$ndx];
...
$list .= <<<EOT
  <div id="d$id" class="gchk" title="3">
    <input type="checkbox"  id="c$id" class="chk" value="$id" name="$code" onclick="gck($id)"/>
    <div id="dc$id" class="descG4" onclick="clickit($id)">&#x2002;&#x2002;$gkey[$ndx] $description</div>
  </div>&#x2003;
...
$ndx++;
  ...
  $Echecks = 'var EcheckBoxes = ' . $foods[1] . '}';
  $Gchecks = 'var GcheckBoxes = ' . $foods[3] . '}';
 $num = 200;
...
.ige{display:inline-block;width: 300px;margin: 5px 0 3px 0;font-size: .9em;  padding: 9.5px 0 9.5px 0;background:#ff0;text-align:center;}
.igg{display:inline-block;width: 300px;margin: 5px 1em 3px 21px;font-size: .9em;  padding: 9.5px 0 9.5px 0;; background:#0ff;text-align:center;}
...
.descE,.descG4{font-size:.9em;width:263px;display:inline-block;font-weight:700;padding: 4.7px 0 6.3px 0;margin: 5px 5px 5px 0;}

.titlebutton,.typebutton,.select,.valuebutton{margin:3px 0 0 .2em;border:0;color:#fff;font:700 1em Arial;height:2em;background: linear-gradient(to bottom, rgb(154, 154, 215) 0%, rgb(2, 2, 36) 100%);padding:0;display:inline-block;}

.panel,.titlebutton,.typebutton,.select,.valuebutton,.blueLG{background: linear-gradient(to bottom, rgba(0,0,255,1) 0%, rgba(0,0,64,1) 100%);} 
...
<div id="msg">
$err
<button id="c0" class="link" type="button" onclick ="exp('10')">Patient Instructions:</button>
<div id="d10">
...
<div>
<input type="hidden" name="rec" value="$rec"/>
<button id="c10" class="link" type="button" onclick ="exp('10')">$patient</button><br>
<div id="d10">
<br>First:<br><input type="text" name="first" value="$first" />
Last:<br><input type="text" name="last" value="$last" />
...
EOT;
  ...
  $jsort =  "var checkBoxes = {10:['none',null,null,false,false,false,false,false,false,false],20:['none',null,null,false,false,false,false,false,false,false],33:['none',null,null,false,false,false,false,false,false,false],";
}
...
<td>&#x2003;</td>
<td><div id="p100" class="titlebutton" onclick="showPanel('100')">$cp[100]</div></td>
</tr>
<tr>
...
<td>&#x2003;</td>
<td><div id="p101" class="titlebutton" onclick="showPanel('101')">$p[101]</div></td>
</tr>
<tr>
...
<td>&#x2003;</td>
<td><div id="p102" class="titlebutton" onclick="showPanel('102')">$p[102]></div></td>
</tr>
<tr>
...
<td>&#x2003;</td>
<td><div id="p103" class="titlebutton" onclick="showPanel('103')">$p[103]</div></td>
</tr>
<tr>
...
<td>&#x2003;</td>
<td><div id="p104" class="titlebutton" onclick="showPanel('104')">$p[104]</div></td>
</tr>
EOT;
$cnt = count($cp) - 5;
$pdx = 105;
...
echo <<<EOT
<button id="c30" class="link" type="button" onclick ="exp('30')">Panel Details</button>
<div id="d30" class="expand">
...
foreach($panels as $type => $array){
  ...
  $idPanels .= "$number:['none',null],";
  $jscp .=  "$number:[null,null],";
  echo <<<EOT
  

<div  class="pchk" >
<input type="checkbox"  id="c$number" class="chk" name="$panelType[$type]$type$panel" value="$panel" />
<div class="inline" onclick="exp('$number')">
<div class="panel">&#x2002;$v[0]</div></div>
<div class="inline" >
<div class="pige">$cntE</div><div class="pigg4">$cntG4</div></div><br></div>
EOT;
   if(count($tests[$number]) > 0){
     asort($tests[$number]);
     echo "<div id=\"d$number\" class=\"expand\" >\n";
...
var prev = document.getElementById('S1');
var panelBackground = [];
...
var ratedCount = 0
var checkBoxBackground = {1:['#ff0'],3:['#0ff']};
var checkBoxBorderColor = {1:['#ff0','#f00'],3:['#0ff','#f00']};
...
var id = 0;
  ...
  for (var id in idPanels){
	idPanels [id][0] = document.getElementById('d' + id);
...
function clearAll(tf,type){
  ...
  for (var id in EcheckBoxes){if ( set[type][1]){break;}
    ...
    EcheckBoxes[id][1].style.background = checkBoxBorderColor[1][0];
	EcheckBoxes[id][1].style.background = checkBoxBorderColor[1][Number(tf)];
  }

  for (var id in GcheckBoxes){if (set[type][3]){break;}
	GcheckBoxes[id][2].checked = tf;
    GcheckBoxes[id][3].style.background = checkBoxBackground[3];
	GcheckBoxes[id][1].style.background = checkBoxBackground[3];
	GcheckBoxes[id][1].style.background = checkBoxBorderColor[3][Number(tf)];
  }
}
function clickit(id){
...
function showtop(cnt){cnt -=2;
  for (var ndx in e){id = e[ndx];console.log('id=' + id + ' cnt=' + cnt);
    if(EcheckBoxes[id][2].disabled){ndx++;continue;}
  ...
  for (var ndx in g){id = g[ndx];console.log('id=' + id);
...
function showPanel(id){//◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙◙
  ...
  idPanels[previousPanel][1].style.background = panelBackground[false];
  idPanels[previousPanel][1].checked = false;
  idPanels[id][1].checked = toggle[check];
  idPanels[id][1].style.background = panelBackground[idPanels[id].checked];
  previousPanel = id;
  pck(id);
}
...
function exp(id){
  idPanels[id][0].style.display=flip[idPanels[id][0].style.display];
  prev=id;
}
