...
if($client == 0 && $sub == 0){  // no POST try GET
...
//ON DUPLICATE KEY UPDATE `client`='$client',`date`='$date',`last`='$last',`first`='$first',`dob`='$dob',`address`='$address',`city`='$city',`state`='$state',`zip`='$zip',`gender`='$gender',id=LAST_INSERT_ID(id)"
...
if( $sub == 64){   // PATIENT DONE SAVE HISTORY
...
if($match){$code='<input type="hidden" name="c" value="' . $c . '"/>';}else {$code = '<br/>Code: <input type="text" name="c" value="' . $c . '" />';}
