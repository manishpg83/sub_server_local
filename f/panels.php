...
.desc{font-weight:700;color:#fff;background-color:#2985EA;text-align:left;width:100%;background-image: linear-gradient(to bottom, #2ef 0%, #02f 100%);}
#pnl{width:4em;}
...
<tr><td><input type="checkbox" name="rec" value="$rec" /></td><td>$last, $first</td><td>$dob</td><td>$date</td><td>
$cnt</td><td><form action="./" method="post"><button>Dashboard</button><input type="hidden" name="sub" value="14" /><input type="hidden" name="rec" value="$rec" /></form></td><td>$rec</td><td><form action="./newIndex.php" method="post"><button>Foods</button><input type="hidden" name="sub" value="4" /><input type="hidden" name="rec" value="$rec" /><input type="hidden" name="client" value="$id" /></form></td></tr>

EOT;
  }
  echo '</table><button name="action" value="3">Delete Checked</button><button name="action" value="1">Test Checked</button></form></div><hr>';
}
