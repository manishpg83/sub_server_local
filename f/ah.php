...
.v5{color:#ff0;}
.c5{color:#3bf;}
.h1,.h2,.h3,.h4,.h5,.h6,#sub,#lo,#ss,#ph,#clear,#announce,#close{font-weight:700;border:0;padding:0;margin:0;color:#fff;background:#66CCCC;
...
.ck1{color:#fff;background-color:#2985EA;text-align:left;padding-left:40px;background-image:url(unchecked.gif);background-repeat:no-repeat;}
...
<div id="page"><h2>All History</h2>
<div><form style="display:block;" action="https://dev.amxemr.com/f/" method="post"><div>
...
<div id="s0">
<table><tr>
<td><button class="c4"  type="submit">Client</button></td>
...
EOT;
  ...
  while (list($rec,$date,$last,$first,$dob,$gender,$id) = @mysqli_fetch_array($results, MYSQLI_NUM)) {
    ...
    echo <<<EOT
<tr>
<td><button class="c4"  type="submit">$id</button></td>
<td><form action="./" method="post"><input type="hidden" name="rec" value="$rec" /><input type="hidden" name="sub" value="4" /><input type="hidden" name="id" value="$id" />
