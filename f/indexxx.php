...
//if($sub == 3){include('/home/isl/public_html/amx/f/request.php');exit;}
...
if($client > 199999 && $client < 1000000){
  $sql = "SELECT `Name`,`passcode`  FROM `Client` WHERE `Number` = $client LIMIT 1";
  $results = mysql_query($sql);
  if(mysql_errno() == 0 && mysql_num_rows($results) > 0){
    list($name,$passcode) = mysql_fetch_array($results, MYSQL_NUM);
    $clientName = "<h3>$name</h3><input type=\"hidden\" name=\"client\" value=\"$client\" />";
    if($sub == 0 && $passcode == $c){$sub = 16;}
  }
}
else{
  $client = 0;
...
if($dobOK === true || $sub == 16){$dobid='';}else{$dobid='id="dob"';}
  ...
  $stateOK = false;
  $state = strtoupper(trim(urldecode($_POST['state'])));
...
if($sub == 4){
  ...
  $sqlf .= "\n" . mysql_error();
  list($client,$date,$last,$first,$dob,$gender,$jsn) = @mysql_fetch_array($results, MYSQL_NUM);
...
header('Cache-Control: max-age=120');
  ...
  if($ndx > 0){$rec = $ndx;}
  $sqlf = "UPDATE `history` SET `client`='$client',`date`='$date',`last`='$last',`first`='$first',\n`dob`='$dob',`address`='$address',`city`='$city',`state`='$state',`zip`='$zip',`gender`='$gender' WHERE `id`=$client";
  mysql_query($sqlf);
...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en"><head><meta http-equiv="Content-Type" content="application/xhtml+xml; charset=utf-8"/><title>Patient Diet Test Selections</title>
<meta name="viewport" content="width=480, initial-scale=1.0" />
<style type="text/css">
body{width:100%;font:700 1em Arial,sans-serif;margin:0;color:#ff0;padding:0;background:#144; }
...
input[type="radio"],input[type="checkbox"]{opacity:1;width:1.5em;height:1.5em;border:1px solid #eee;outline:2px solid #eee;display: inline;margin:1px;margin:2px 0 2px 2px;vertical-align: middle;position: relative;color:#eee;   display: inline;vertical-align: middle;position: relative;}   
input[type="text"]{font:700 1em Arial,sans-serif;}
.hit{color:#f00;}
.grp{float:left;padding:.1em;width:19em;}
.fchk{cursor:pointer;position:relative;text-align:left;background:#2985EA;padding:0;margin:.1em 0 ;color:#fff;display:inline-block;font:700 1em Arial,sans-serif;}
...
.descE,.descG4{font-size:.9em;width:12em;display:inline-block;font-weight:700;padding:0;margin-right:.4em;}
...
.ige,.igg4{display:inline-block;width:40%;margin: .6em 1em 0;}
...
.titlebutton,.typebutton,.select,.valuebutton{font:700 1em Arial;}
.titlebutton{margin:3px 0 0 .2em;color:#fff;background-color:#006;padding:0;vertical-align:top;border:2px outset #fff;display:inline-block;width:170px;height:25px;}
.valuebutton{margin:0;font:700 1em Arial;color:#fff;background-color:#144;padding:0;vertical-align:top;border:0;display:inline-block;width:50px;height:25px;}
.typebutton{margin:3px 0 0 .2em;color:#fff;background-color:#006;padding:0;vertical-align:top;border:2px outset #fff;display:inline-block;width:50px;height:25px;}
.select{margin:3px 0 0 .2em;color:#fff;background-color:#006;padding:0;vertical-align:top;border:2px outset #fff;display:inline-block;width:112px;height:25px;}
...
elseif($sub == 0){
  if($action == 1 && strlen($c)== 4){
    ...
    if(mysql_errno() == 0 && mysql_num_rows($results) == 1){
      ...
      echo "<h3>$name</h3><input type=\"hidden\" name=\"client\" value=\"$client\" />";
      if($passcode == $c){$sub = 16;}
    }
  }


if($client == 0){$s = 0;}else{$s = 8;}
  echo <<<EOT
  $err
<div class="center">
<div id="post"><form action="#" method="post"><div id="patient">&#x2003;
<br/>Code: <input type="text" name="c" value="$c" />
...
if(($sub & 17) && ( $len < 3 || $gender == 0 || strlen($strdob) == 0)){
...
<br/><br/>DoB: m&thinsp;/&thinsp;d&thinsp;/&thinsp;yyyy&#x2003;<input $dobid type="text" name="dob" value="$strdob" placeholder="m/d/yyyy" /><br/><br/>
...
<br/><br/>Address: <br/><input name="address" type="text" value="" /></p>
City: <br/><input name="city" type="text" value="" /></p>
...
Zip: <br/><input name="zip" type="text" value="" /></p>
<p>&#x2003;<br/>Enter Your Name, Date of Birth, Gender, and Address</p>
...
if($sub == 1 && $dobOK){
  echo'<div id="col"><form action="https://dev.amxemr.com/h/" method="post"><div id="post">';
...
<div id="d022" class="dchk"><input type="checkbox"  id="c022" class="chk" name="F022" value="356" onclick="ck('022')" $checked[022]/>&#x2002;Bean, Carob </div>
...
elseif($sub == 4){
  ...
  $s14 = 'block';
    ...
    $sql = "SELECT `Code`, `shortdescription` FROM `Rast` WHERE `Code` LIKE'F%' ORDER BY `Code`";  
    $results = mysql_query($sql);
    while(list($code,$description) =  mysql_fetch_array($results, MYSQL_NUM)){$desc[$code] = $description;$descriptions[$code][1] = 0;$rank[10][$code] = $num;$num++;}
...
//   echo "<p>$ke=>$e | $kg4=>$g4</p> ";
   ...
   $jsort .= "$cg4],";
   $list .= <<<EOT
<div class="row"><div id="$clientde" class="fchk" title="1"><input type="checkbox"  id="$clientce" class="chk" value="$ke" name="E$numE" onclick="ck('$ce')" $checkedE[$ke] /><div class="descE" onclick="clickit($ce)">&#x2002;$ndx&#x2002;$desc[$ke]</div></div>&#x2003;<div id="$clientdg4" class="fchk" title="3"><input type="checkbox"  id="$clientcg4" class="chk" value="$kg4" name="G$numG4" onclick="ck('$cg4')" $checkedG[$kg4] /><div class="descG4" onclick="clickit($cg4)">&#x2002;$ndx&#x2002;$desc[$kg4]</div></div> <br/> </div>
EOT;
    next($showE);
    next($showG4);
  }
...
while(list($number,$description,$panel) =  mysql_fetch_array($results, MYSQL_NUM)){
  ...
  $panels[0][$pdx] =array($description,"$hits[1]/$TestTypeCount[1]","$hits[3]/$TestTypeCount[3]",$hits,$panel);
...
<button id="b16" class="link" type="button" onclick ="panels('s16')">Patient Diet Test Selections</button>
<div id="s16" class="expand">
<table>
<tr>
<td><button id="b2000" type="button" class="titlebutton" onclick="showPanel('2000')">Comprehensive</button></td>
<td><button id="b2001" type="button" class="typebutton"  onclick="showPanel('2001')">IgE</button></td>
<td><button id="b2002" type="button" class="typebutton"  onclick="showPanel('2002')">IgG4</button></td>
<td>&#x2003;</td>
<td><button id="b1000" type="button" class="titlebutton" onclick="showPanel('1000')">$desc[1000]</button></td>

</tr>
<tr>
<td><button id="b2003" type="button" class="titlebutton" onclick="showPanel('2003')">Standard</button></td>
<td><button id="b2004" type="button" class="typebutton"  onclick="showPanel('2004')">IgE</button></td>
<td><button id="b2005" type="button" class="typebutton"  onclick="showPanel('2005')">IgG4</button></td>
<td>&#x2003;</td>
<td><button id="b1001" type="button" class="titlebutton" onclick="showPanel('1001')">$desc[1001]</button></td>

</tr>
<tr>
<td><button id="b2006" type="button" class="titlebutton" onclick="showPanel('2006')">Mini</button></td>
<td><button id="b2007" type="button" class="typebutton"  onclick="showPanel('2007')">IgE</button></td>
<td><button id="b2008" type="button" class="typebutton"  onclick="showPanel('2008')">IgG4</button></td>
<td>&#x2003;</td>
<td><button id="b1002" type="button" class="titlebutton" onclick="showPanel('1002')">$desc[1002]</button></td>

</tr>
<tr>
<td><button id="b2009" type="button" class="titlebutton" onclick="showPanel('2009')">IBS</button></td>
<td><button id="b2010" type="button" class="typebutton"  onclick="showPanel('2010')">IgE</button></td>
<td><button id="b2011" type="button" class="typebutton"  onclick="showPanel('2011')">IgG4</button></td>
<td>&#x2003;</td>
<td><button id="b1003" type="button" class="titlebutton" onclick="showPanel('1003')">$desc[1003]</button></td>

</tr>
<tr>
<td><button id="b2012" type="button" class="titlebutton" onclick="showPanel('2012')">Hidden Foods</button></td>
<td><button id="b2013" type="button" class="typebutton" onclick="showPanel('2013')">IgE</button></td>
<td><button id="b2014" type="button" class="typebutton" onclick="showPanel('2014')">IgG4</button></td>
<td>&#x2003;</td>
<td><button id="b1004" type="button" class="titlebutton" onclick="showPanel('1004')">$desc[1004]</button></td>
...
<button id="c007" type="button" class="select" name="C" value="999" onclick="showtop('007',999)">&#x2002;All</button>
<div>
  <div class="ige">IgE </div><div class="igg4">IgG4</div>
  </div>$list
EOT;
ob_flush();
...
while (($text= fgets($fp , 72)) !== false) {
  ...
  $panels[1][$pdx] =array($description,"$hits[1]/$TestTypeCount[1]","$hits[3]/$TestTypeCount[3]",$hits,$panel);
  $codes = substr($codes,0,-1) . ']';
  $jspc .= "$pdx:[$codes,";
  $pdx++;
}
...
foreach($panels as $type => $array){
  ...
  $js .= "$number:[null,null,null,null,'$title','$cntE','$cntG4'],";
  echo <<<EOT
<div id="P$number" class="pchk">
...
function ck2(id){
  f[id].style.backgroundColor=bg[cf[id].checked];
  v[id].style.backgroundColor=bg[cv[id].checked];
}
function clickit(id){ 
...
function showPanel(p){
  if (foodPanels[p] != null){
    foodPanels[p][1].checked = toggle[foodPanels[p][1].checked];
    pck(p);
  }else{
    alert(p);
  }
...
function showtop(on,off){
  ...
  for (var iType=1; iType<4; iType+=2){
    var ndx = 0;
    for (var div=0; div<RatedSort.length; div++){
     if(Rated[RatedSort[div][iType]][1].disabled){ndx++;continue;};
      if(ndx < off){
        Rated[RatedSort[div][iType]][0].style.backgroundColor=bg[true];
        Rated[RatedSort[div][iType]][1].checked=true;
        Rated[RatedSort[div][iType]][1].disabled=false;
        Rated[RatedSort[div][iType]][1].style.opacity=1;
      }
      else{
        Rated[RatedSort[div][iType]][0].style.backgroundColor=bg[false];
        Rated[RatedSort[div][iType]][1].checked=false;
        Rated[RatedSort[div][iType]][1].disabled=false;
        Rated[RatedSort[div][iType]][1].style.opacity=1;
      }
      ndx++;
  ...
  ratedCount = off;
 // C.style.backgroundColor=bg[false];

}

function clearIgX(X){
  for (var div=0; div<RatedSort.length; div++){
      if(Rated[RatedSort[div][X]][1].disabled){continue;};
	  Rated[RatedSort[div][X]][0].style.backgroundColor=bg[false];
	  Rated[RatedSort[div][X]][1].checked=false;
	  Rated[RatedSort[div][X]][1].disabled=false;
      Rated[RatedSort[div][X]][1].style.opacity=1;
  }
}
function clearAll(){ 
  ...
  for (var r in foodPanels){
    foodPanels[r][0].style.backgroundColor = bg[false];
    foodPanels[r][1].checked = false;
    if(foodPanels[r][3] == null){alert('panel=' + r);continue;}
    foodPanels[r][3].style.backgroundColor = '#006';	
  }
  foodPanels[2000][1].checked = false;
  pck(2000);
...
//div.sort;
  for (div=0; div<divs.length; div++){
    ...
    if (did != null){
      ...
      if (idType == "b"){
	    i = did.substring(1,6);
		if (foodPanels[i] != null){
		  divs[div].style.display = 'block';
	      foodPanels[i][3] = divs[div]; 
		}
	  }
	}
  }
...
//  if(ratedCount < 16){document.getElementById('d004').style.display = 'none';}
  document.getElementById('d001').style.backgroundColor = bg[document.getElementById('c001').checked];
  document.getElementById('d002').style.backgroundColor = bg[document.getElementById('c002').checked];
...
function ckf(id){
  ...
  df.style.backgroundColor=bg[cf.checked];
  document.getElementById('v' + id).style.backgroundColor=bg[cf.checked];
  document.getElementById('cv' + id).checked = cf.checked;
}
function ckv(id){
  var dv = document.getElementById('v' + id);
  var cv = document.getElementById('cv' + id);
  dv.style.backgroundColor=bg[cv.checked];
  document.getElementById('f' + id).style.backgroundColor=bg[cv.checked];
