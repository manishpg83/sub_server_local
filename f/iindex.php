...
if($rec > 0){
  $sqlf = "SELECT `client`,`date`,`last`,`first`,`dob`,`address`,`city`,`state`,`zip`,`gender`,`history`,`foods` FROM `history` WHERE `id` = $rec";
...
.ige,.igg4{display:inline-block;width:35%;margin: .6em 1em 0 1.6em;}
...
<div id="s16" class="expand" style="background:#132;">
<table>
<tr>
  ...
  for (div=0; div<divs.length; div++){
    ...
    if (did != null){
      ...
      if (idType == "R"){
        ...
        if(foodPanels[i][2] != null){
          foodPanels[i][2].style.display = 'none';
		}
		if(foodPanels[i][0] != null){
