...
//  else{
    ...
    $id = intval($_POST['id']);
    if ($id == 0 && strlen($passcode) == 12  ) {
      $id = intval(substr($passcode,3,6));
    }
//  }
  if ((hash('ripemd320',$passcode) == '6732f3c024fe3c3c1ccd7dbe5d0fa7d4a53c516f800707bb86a9017c1e1646d9d4a2d7adc56a505d') ){ //&& (substr($ip,0,11)  == '128.30.52.7' || $ip == '99.3.150.55' || $ip == '173.162.23.21' || $ip == '70.197.160.30')){
    $admin = true;
    $clear = '<div id="sc"><form action="https://amxemr.com/emr/" method="post"><input type="hidden" name="id" value="' . $SAVE_ID . '"><input type="hidden" name="pw" value="' . $passcode . '"><input type="hidden" name="sub" value="9"><button id="clear">Reset Diet Icons</button></form></div>';  
  }
  $sql = "SELECT COUNT(*)  FROM `Banned` WHERE `ip` LIKE '$ip'";
  $results = mysql_query($sql);
  $row = mysql_fetch_array($results, MYSQL_NUM);
  if ($row[0] > 0){include 'new.php';return;}
  $pass = false;
  $error = 1;
  ...
  if (($statusSort & 8) > 0){$chkStatus[3] = ' checked="checked"';}
   ...
   include('/home/amx/public_html/index.php');return;
   $admin = false;
  ...
  if ($id < 100000){include('/home/amx/public_html/new.php');return;}
  setcookie("amxc", $id,time()+86400);
  if ($sub == 4){
    $name = $_POST['name'];
  }
...
.dietblack,.dietred,.dietblue,.none{vertical-align: middle;border:0;margin:0;padding:0;width:50px;}
.none{background-image: url('data:image/gif;base64,');}
.dietblack{background-image: url('data:image/gif;base64,R0lGODlhMgAoAJEAAAIBAvb29h4dHv3+/SwAAAAAMgAoAAAChISPqcvtD6OctNqLs76C9f0MgEguIuiUZRKcKKOOyUC/TeyOdG4juL/rmWQrw44nJCoPx+TsWGMGnVIoMErVWavIXmfLzV4/YbERWzZPr93kOi1+n9FZ+dx8N7WF9jNeC7PX06fzR0hHdShoo2iI6IfXGPlYONlGyUc5EPDX6fkJGupUAAA7');}
.dietblue  {background-image: url('data:image/gif;base64,R0lGODlhMgAoAJH/AAAA//////3+/fb29iwAAAAAMgAoAAACgoSPqcvtD6OctNqLs94p8CcA4biEX0OSyWCepQi3hkC7TAofdG0ruLyT9Wax3Gw3TPwQwSQzyDtGndBp0ym9ZrE67ZYr9SHBoukXPH6aqWsvNt1dJ+FxclnYtdPz5P1RL1cGiPfXFxjY4ydoSLiIdtiYCDl4E2mjKDBgt8nZ6fmZVAAAOw==');}
.dietred {background-image: url('data:image/gif;base64,R0lGODlhMgAoAJEAAOQAE/3+/eYYKukxQCwAAAAAMgAoAAAChoSPqcvtD6OctNqLs94p8OeFgKeQHyOKnXmW48uOQdwe6WvPdA33+cxz3X672o2lK7aSOkQyuGI6m1Ai1fCsWqPKiEByJWoGYCB3XKZlMeRIGPuufN3maV0r28Whexn+h9K1dDf1B1dEWNWXyMfoZ7goeBJpmBdYSQnp6MiTWfkJGio6ylMAADs=');}
...
.v5{color:#ff0;}
.c5{color:#3bf;}
.h1,.h2,.h3,.h4,.h5,.h6,#sub,#lo,#ss,#clear,#announce,#close{font-weight:700;border:0;padding:0;margin:0;color:#fff;background:#66CCCC;
...
#sub,#ss,#clear,.box,#lo,#announce,#close{border:2px ridge #6cc;margin-top:4px;}
...
#lo,#announce{width:48%;display:inline-block;height:2.5em;}
#daysValue{width:4em;font:700 1em Arial,sans-serif;}
#diet,#announceTxt{border:.5em ridge #fff;padding:2em;background:#222;color:#fff;position: absolute;top:20em;z-index:10;display:none;}
...
table,#frmtop,#frmbot{border:0;margin:0 auto 0 ;}
#frmtop,#frmbot{padding:.25em;background:#144;color:#69B5B3;font:700 1em Arial,sans-serif;}
#msg{color:#ff0;}
...
<div id="frmtop">
<form action="https://amxemr.com/logout.php">
...
<form action="https://amxemr.com/emr/" method="post">
...
<input type="number" id="daysValue" min="1" max="999" size="2" name="d" value="$days" />
&#x2002;( 5 days / week)</div>
...
$sort = $sort[$sortVal];
$hid[0] = '<tr><td><button class="h1"  type="button" name="sn1" onclick="stNum(1)" >Name</button></td><td><button class="h3"  type="button" name="sn3" onclick="stNum(3)" >AMX Lab ID</button></td><td><button class="h4"  type="button" name="sn4" onclick="stNum(4)" >Date</button></td><td><button class="h5"  type="button" name="sn5" onclick="stNum(5)" >Status</button></td><td class="h6"><button class="h6" type="button" name="sn6" onmouseover="mouseOverDiet(event)"  onmouseout="mouseOutDiet()" >Diet</button></td></tr>';
$hid[1] = '<tr><td><button class="h1"  type="button" name="sn1" onclick="stNum(1)" >Name</button></td><td><button class="h2"  type="button" name="sn2" onclick="stNum(2)" >Patient ID</button></td><td><button class="h3"  type="button" name="sn3" onclick="stNum(3)" >AMX Lab ID</button></td><td><button class="h4"  type="button" name="sn4" onclick="stNum(4)" >Date</button></td><td><button class="h5"  type="button" name="sn5" onclick="stNum(5)" >Status</button></td><td><button class="h6"  type="button" name="sn6" onmouseover="mouseOverDiet(event)"  onmouseout="mouseOutDiet()" >Diet</button></td></tr>';
...
$pdf['C'][1] = '<td><form action="https://amxemr.com/pdf.php" method="get" target="_blank"><button class="c5"  type="submit">Complete ( PDF )</button><input type="hidden" name="p" value="';
$pdf['V'][2] = '</button></td>';
$pdf['L'][2] = '</button></td>';
$pdf['Q'][2] = '</button></td>';
$pdf['X'][2] = '</button></td>';
...
if ($rows == 0){echo "<h2 id=\"msg\"><br/> &#X2003; No $type<br/>&#x2002;</h2></div>";$showSearch = 'block';}
  ...
  echo "<h4 id=\"msg\" onclick=\"hideSearch()\">&#X2003; Today: $today<br/>&#X2003; $type<br/>&#X2003; $moreRows[0] Patients Found: " . $statusCnt['C'] . ' Complete, '  . $statusCnt['T'] . ' Testing, '  . $statusCnt['R'] . ' Received</h4>';
  echo '<button id="ss" type="button"  onclick="showSearch()" >Show Search</button></div>';
  echo '<div id="diet"><div class="dietblack"></div><p class="inline"> &#x2003;Black: diet report not generated.</p><br/><div class="dietblue"></div><p class="inline"> &#x2003;Blue: Diet Report previously generated</p><br/><div class="dietred"></div><p class="inline"> &#x2003;Red: Multiple result reports linked</p><br/><div class="none"></div><p class="inline"> &#x2003; No Food, Grass, Tree, or Weed found for diet</p></div><table id="tbl">' . $hid[$cid];
  flush();
  ob_flush();
  $exStatus['X'] = true;
...
//	$age = date_diff(date_create($row[9]), date_create('today'))->y;
    ...
    echo <<<EOT
<tr>
<td><form action="./" method="post"><input type="hidden" name="sub" value="4" /><input type="hidden" name="name" value="$row[6]" /><button class="c1"  type="submit">$row[6], $row[7]</button></form></td>
$idRow[$cid]
...
while(true){div++;if (div > 8){break;}d[div] = document.getElementById('d' + div);if (d[div]==null){continue;}c[div] = document.getElementById('c' + div);if (c[div]==null){alert('Null CHK: ' + div);continue;}if (c[div].checked){c[div].style.backgroundColor=bg[1];d[div].style.backgroundColor=bg[1];}}
...
function init(){document.getElementById('inp').style.display='$showSearch';document.getElementById('ss').style.display='block';var len = document.getElementById('tbl').offsetWidth;len-=10;document.getElementById('frmtop').style.width = (len + 'px');document.getElementById('frmbot').style.width = (len + 'px'); len-=80; at.style.width = (len + 'px'); }
function showSearch(){document.getElementById('inp').style.display='block';document.getElementById('ss').style.display='none';document.getElementById('msg').style.display='none';}
function hideSearch(){document.getElementById('inp').style.display='none';document.getElementById('ss').style.display='block';}
