...
h4 {color:#900000;text-align:left;font-size:1.2em;font-family:Arial,serif;border:0;margin:0;padding:0;}
...
#search{background:#9ec8cf; width: 90%; text-align: center;  padding:20px 0 20px 0; margin:0 auto 0; }
img { float:left;border:0;}
.main {background:#9ec8cf; width: 90%; text-align: left;  padding:20px 0 20px 0; margin:30px auto 0; }


.color{ text-align:center;margin:0.1em;font-size:0.1em; }
#menu{background-color:#9ec8cf;margin:0 0 0 0;width:100%;font-size:1.5em;}
...
<div id="links">
<p><br/></p>
<h2>Recent Activity for the past ' . "$adays" . ' days</h2> 
<table>
<tr><th>View</th><th class="status">Status</th><th class="date">Expected/Complete</th><th>ID</th><th class="pat">Patient Name</th>
</tr>' ;
  print "\n";
  ...
  WHILE ($row = mysqli_fetch_array($results, MYSQLI_NUM)) {
    ...
    $date = date ( "F j, Y" , $row[1] );
      echo '<tr><td class="details">' . "$pdf " . '</td><td class="status">' . " $status " . '</td><td class="date">' . "$row[2]" . '</td><td>' . "$row[9]" . '</td><td class="pat">' . "$row[3] $row[4]</td>
	  </tr>\n" ;
