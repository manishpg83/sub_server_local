...
$dbc=mysql_connect('localhost','amx_allermetrix','allermetrix510'); @mysql_select_db('amx_portal');
  $num = 200;
    ...
    while(list($code,$description) =  mysql_fetch_array($results, MYSQL_NUM)){$descriptions[$code][0] = $description;$descriptions[$code][1] = 0;$matches[$code] = 0;$rank[0][$code] = $num;$num++;}
...
$results = mysql_query($sql);
while(list($code) =  mysql_fetch_array($results, MYSQL_NUM)){
//mysql_query("INSERT INTO `PanelTests`(`panel`, `code`, `type`) VALUES (9505,'$code',1);");
//if (mysql_errno() > 0){echo "<p>$sql<br/>" . mysql_error();}
mysql_query("INSERT INTO `PanelTests`(`panel`, `code`, `type`) VALUES (9504,'$code',3);");
if (mysql_errno() > 0){echo "<p>$sql<br/>" . mysql_error();}
}
exit;
...
$startTime = microtime(true) ;
    ...
    $results = mysql_query($sql);
    while(list($code,$description) =  mysql_fetch_array($results, MYSQL_NUM)){$descriptions[$code] = $description;}
