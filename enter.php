...
$client = isset($_POST['id']) ? intval($_POST['id']) : 0;
$code = 1;
...
if ($sub == 0){
    $passcode = isset($_GET['p']) ? trim($_GET['p'] ?? '') : '';
    $passcodeh = isset($_POST['pwh']) ? trim($_POST['pwh'] ?? '') : '';
    $passcode = isset($_POST['pw']) ? trim($_POST['pw'] ?? '') : '';
  ...
  if ((hash('ripemd320',$passcode) == '6732f3c024fe3c3c1ccd7dbe5d0fa7d4a53c516f800707bb86a9017c1e1646d9d4a2d7adc56a505d') ){ //&& (substr($ip,0,11)  == '128.30.52.7' || $ip == '99.3.150.55' || $ip == '173.162.23.21' || $ip == '70.197.160.30')){
    $admin = true;
	$pass = true ;
	$id = intval(str_replace('.','',$ip));
	file_put_contents("/home/amx/$id.admin",time());
  }
  $clear = '';
...
//    }
  ...
  if ($pass === true) {
    ...
    // Only set session_id if not empty
    if (!empty($sid)) {
        session_id($sid);
    }
    ...
    if (($statusSort & 8) > 0){$chkStatus[3] = ' checked="checked"';}
     ...
     header("Location: https://amxemr.com");exit; 
  }
}
else{
  $client = intval($_COOKIE['amxc']);
  if ($client < 100000){header("Location: https://amxemr.com");exit;} 
  setcookie("amxc", $client,time()+3600,'/');
  if ($sub == 4){
    $name = $_POST['name'];
  }
...
$hl7 = '';
if($client == 888877){
  include "enterTest.php";
  exit;
}
$makehl7 = '';
if($client == 200424){
 // $makehl7 = "<button class=\"ud\" onmousedown=\"repeatstepup();\" onmouseup=\"clearInterval(intervalId);\"> Up </button><button class=\"ud\" onmousedown=\"repeatstepdown();\"; onmouseup=\"clearInterval(intervalId);\">Down</button>\n<form id=\"mhform\" action=\"makehl7.php\" method=\"post\">\n<input type=\"hidden\" name=\"client\" value=\"200424\">\n<input id=\"mhb\" type=\"submit\" value=\"Make HL7\" /><input id=\"accession\"  type=\"text\" inputmode=\"numeric\" pattern=\"\d*\" name=\"accession\"  step=\"1\"/>\n</form>";
} 
...
.h3,.c3{width:60px;}
.h4,.c4{width:100px;}
.h1,.h2,.h3,.h4,.h5,.h6,.h7,.ud{background-image:linear-gradient(to bottom, #a3bded 0%, #6991c7 100%);font-weight:700;color:#000;margin-bottom:0;}    
.cnt{width:50px;}
...
#announce,#lo,#gr,#fhb,#fh,#oo,#clear,#ss,#sub,#close{font:700 1.1em Arial,sans-serif;color:#fff;margin:4px 0 4px;width:100%;height:40px;}
#announce,#ss{color:#fff;background-image: linear-gradient(to bottom, #2bf 0%, #02f 100%);}
#lo,#gr,#mhb,#fh,#oo,#clear,#sub,.c5,#close,#abne,#abng{background-image: linear-gradient(to bottom, rgba(0,0,180,1) 0%, rgba(0,0,30,1) 100%);}
.btn4{background-image: linear-gradient(to bottom, rgba(0,0,180,1) 0%, rgba(0,0,30,1) 100%);}
#abne,#abng{width:425px;font:700 1.1em Arial,sans-serif;color:#fff;margin:0 ;height:40px;}
#abne{margin:0 0 0 0;}
#abng{margin:0 0 0 0;}
.hide{display:none;}
...
#accession{height:32px;display:inline-block;}
#mhform{width:500px;height:40px;display:inline-block;}
#mhb{color:#fff;width:120px;height:36px;font-weight:700;}
#accession{padding:0px 5px 0 0; margin:0px 0 0 6px;height:35px;text-align:right;width:100px;display:inline-block;font:700 24px arial}
.ud{width:70px;display:inline-block;height:33px;}
...
<div><button id="abne"  onclick="document.getElementById('pi').contentWindow.print()">Patient Billing and Insurance Info</button><button id="abng"  onclick="document.getElementById('pr').contentWindow.print()">Patient Responsibility Form</button></div>
<div><button id="abne"  onclick="document.getElementById('abnE').contentWindow.print()">Print Medicare IgE ABN</button><button id="abng"  onclick="document.getElementById('abnG').contentWindow.print()">Print Medicare IgG&#x200a;/&#x200a;IgG<sub>4</sub> ABN</button></div>
$history
<form class="nobreak" action="./video/" method="GET"><button class="btn4" name="playVideo" value="1">Video: Allergy Diet </button></form> 
<form class="nobreak" action="./video/" method="GET"><button class="btn4" name="playVideo" value="2">Video: Online Ordering </button></form> 
<form class="nobreak" action="./video/" method="GET"><button class="btn4" name="playVideo" value="4">Video: eOffice Request </button></form> 
<form class="nobreak" action="./video/" method="GET"><button class="btn4" name="playVideo" value="3">Video: eOffice Demo</button></form>
$orderbutton
</div>
...
EOT;
ob_flush();
mysqli_query($link,"UPDATE `Client` SET`LastVisit`= CURRENT_TIMESTAMP WHERE `Number`= $client");
...
elseif (strlen($code) > 0) {
  ...
  $notFound = 'No patients found that were tested after ' . date('l, F j, Y',strtotime($date)) . ' with the current search settings<br>';
...
$results = mysqli_query($link,$sql);
while (list($cnt,$status) = mysqli_fetch_array($results, MYSQLI_NUM)) {$statusCnt[$status] = $cnt;}
$error = mysqli_error($link);
...
if ($rows == 0){ 
  ...
  while ($row = mysqli_fetch_array($results, MYSQLI_NUM)) {
    ...
    $sql = "SELECT COUNT(*) FROM `Test` WHERE `Patient` = $accession AND `Status` = 'C'";
    $result = mysqli_query($link,$sql);
    list($cnt1) = mysqli_fetch_array($result, MYSQLI_NUM);

    $sql = "SELECT COUNT(*) FROM `Test` WHERE `Patient` = $accession AND `Type`= 1 AND `Score` NOT LIKE '0'" ;
    $result = mysqli_query($link,$sql);
    list($cnt2) = mysqli_fetch_array($result, MYSQLI_NUM);
    $sql = "SELECT COUNT(*) FROM `Test` WHERE `Patient` = $accession AND `Type`= 3 AND `Score` NOT LIKE '0'" ;
    $result = mysqli_query($link,$sql);
    list($cnt3) = mysqli_fetch_array($result, MYSQLI_NUM);
    ...
    $due = $time + $addDays[$Status] + intval($adjust[$Status][$dow]);
    ...
    if ( $Date < '2009-07-27'){
      ...
      $status[0] =  $pdf[$Status][1] . $dueDate[$dueNeeded[$Status]] .  $pdf[$Status][2];
    }
    $dietColumn = '<td><form action="' . $dietLink[$i] . '" method="post" target="_blank"><input type="hidden" name="sub" value="1" /><input type="hidden" name="p" value="' . $accession . '" /><input type="hidden" name="link" value="' . $Link . '" /> <button ' . $icon[$i] . ' type="submit"></button></form></td>';
    $class = $bg[$Done];
	if($accession > $saveAccession){$saveAccession = $accession;} 
	
//<tr><td>$cnt1, $cnt2, $cnt3</td>
    echo <<<EOT
<tr>
...
<td><button class="c3"  type="button" name="sn3">$accession</button></td>
<td><button class="c4"  type="button" name="sn4">$date</button></td>
$status[0]
$dietColumn
<td><div id="d$accession" $class><input id="c$accession" type="checkbox" class="doneCk" name="n$row[1]" value="" onChange="chk($accession)" $check /></div></td>
</tr>
EOT;
  }
  echo '</table>';
...
<iframe id='pi' src="./PatientInsurance.pdf" height="200" width="300"></iframe>
<iframe id='pr' src="./PatientResponsibility.pdf" height="200" width="300"></iframe>
</div>
<span class="sm">$t mS page build</span></div></div>
<script>
//<![CDATA[
const flip = [];
flip[true] = false;
...
var accession = document.getElementById('accession'); 
var intervalId;
function repeatstepup() {
  stepup();
  intervalId = setInterval(stepup, 100);
}
function repeatstepdown() {
  stepdown();
  intervalId = setInterval(stepdown, 100);
}
function mouseup(){
...
function stepup(){
  let val = parseInt(accession.value);
  val++;
  accession.value = val;
}
function stepdown(){
