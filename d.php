...
<div id="content"><div style="display:none;">$ip</div>
...
if (isset($patient) && strlen($patient) == 6){
  ...
  $rows = @mysqli_num_rows($results);
 //echo "c=$clientNum p=$patient r=$rows $error"; exit;
  ...
  if (strlen($client) == 6){
    if ($sub > 1){
      ...
      $sql = "UPDATE `template` SET `CutOff` = $tpl[2],`CutOffG` = $tpl[13],`excludeMethod` = $tpl[3],`CrossreactivityGrocery` = $tpl[4],`CrossreactivityDiet` = $tpl[5],`CrossreactivityPhycian` = $tpl[6],`CrossreactivityPatient` = $tpl[7],`HiddenFoodPhysician` = $tpl[8],`HiddenFoodPatient` = $tpl[9] WHERE `Client` = 0 AND `Patient` = $patient";
      $results = @mysqli_query($dbc,$sql);
        $error = mysqli_error($dbc);
        if (strlen($error) > 0){
...
//  HEADER AND GLOBALS 
    ...
    if (strlen($client) == 6 AND strlen($patient) == 6){
      ...
      if (mysqli_errno($dbc) == 1146){
       ...
       mysqli_query($dbc,"INSERT INTO $foodsTable SELECT * FROM `Foods999999` ORDER BY `alpha` ASC,`Description` ASC");
      ...
      if ($sub == 2){
        foreach($patients as $key=>$value){
          @mysqli_query($dbc,"UPDATE `Test` SET `Exclude` = 0 WHERE `Patient` = $value");
          @mysqli_query($dbc,"DELETE FROM `Exceptions` WHERE `Patient`= $value");
        }
        foreach ($_POST as $k => $v){
          if (substr($k,0,1) == 'e'){
            $code = substr($k,2);
             foreach($patients as $key=>$value){
               @mysqli_query($dbc,"UPDATE `Test` SET `Exclude` = 1 WHERE `Patient` = $value AND `Code` = '$code' ");
             }
          }
 ...
 //         0    1 v0     2 v6  3 v7 4 v8  5 v9
      ...
      while ($fd = mysqli_fetch_array($results, MYSQLI_NUM)){
        list( $Code,$Description,$alpha,$Day,$Group,$id) = $fd;
        $code2id[$Code][] = $id;
        if (isset($foods[$id])){continue;}
        $grp = intval($Group);
        $day = intval($Day);
        $foods[$id] = array($Description,0,0,0,0,0,$grp,$day,$Group,$Code,'','','',0);
        if ($grp == 0 || ($alpha & 8 || $Day == 0) ){continue;}
...
//###############  COLMBINE PATIENT AND FOODS  ##########################
      ...
      foreach ($patients as $p){
       ...
       while ($row = @mysqli_fetch_array($results, MYSQLI_NUM)){
         ...
         $pos = 14;
         foreach ($code2id[$Code] as $k => $id){
           if($id ==0){continue;} 
           $ndx = $Type+9;
           $foods[$id][$ndx] = $Score;
           $foods[$id][$Type] = $getPoz[$cutoffType[$Type]][$Score];
           $scores = $foods[$id][1] | $foods[$id][2] | $foods[$id][3];
           $foods[$id][5] = $poz[$scores];
           $inc = intval($Exclude); 
           $foods[$id][4] |= $setInc[$inc][$foods[$id][5]];
           if ($Code == 'G006'){$pos++;$foods[441][20] = $foods[441][5] ;}
         }
       }
...
// ###################   COMPONENT FOODS #######################
    ...
    if ($foods[290][5] == 2){
      $component = array(282,283,284,481,474,285,497,487,286,287,288,289,290,296,297 );
    ...
    if ($foods[298][5] == 2 || $foods[299][5] == 2 || $foods[300][5] == 2 || $foods[301][5] == 2 || $foods[302][5] == 2 || $foods[478][5] == 2){
     $component = array(299,300,301,302,478);
        foreach ($component as $v){
         $components['Egg'][] = $v;
      $foods[$v][4] |= $priority[$foods[$v][5]];
         $foods[$v][13] = 1;
...
<div class=\"checkmark\"></div><p class=\"tested\">&#x2009;Positive Result, doctor included in diet&#x2003;</p>
<div class=\"up\"></div><p class=\"tested\">&#x2009;Untested Component of Positive Food, Avoid&#x2003;</p>
<div class=\"tblank\"></div><p class=\"notTested\">&#x2009;Untested</p>";
...
// ob_flush();  
      ...
      foreach ($groups as $k => $v ) {
       ...
       echo '</div></div><div class="xpgBrk"></div>'; 
    }
...
*/
    ...
    if ($tpl[6] == 1){  // FOOD FAMILIES 
      ...
      $savGrp = 99;
      $sql = "SELECT `id`, `Family`,`alpha`,`Description`,`Type` FROM $foodsTable WHERE `alpha` > 0 AND `Family` > 0 ORDER BY $foodsTable.`Family` ASC";
      $results = @mysqli_query($dbc,$sql);
      while(list($id,$Family,$alpha,$Description,$Type) = mysqli_fetch_array($results, MYSQLI_NUM)){
        if ($alpha == 8){continue;}
        if ($savGrp != $alpha){
          $savGrp = $alpha;
          echo $pgBrk[$alpha]; 
          echo "\n<p class=\"title\">&#x2003;$cat[$alpha]</p><div class=\"box\">\n<div class=\"col\">\n";
        }
        if ($Type == 0){
          echo "<p class=\"hd\">&#x2002;$Description&#x2002;</p>\n";
          continue;
        }
        echo $icon[0][$foods[$id][5]] . $Description . "</p><br/>\n";  //Food Families and below
      }
      echo '</div></div>';
      $pg += 3;
    }
...
EOT;
    ...
    if($foods[299][5] == 2 || $foods[301][5] == 2 ||$foods[300][5] == 2 ||$foods[302][5] == 2 || $foods[478][5] == 2 ){ 
     $codes = array('F001','F075','F245','F357','F395');
      $desc = array('Egg White','Egg Yolk ','Egg, Whole','Ovomucoid (egg white)','Ovomucoid');
      $boxes++;
...
EOT;
  ...
  if ($tpl[8] == 1){ //  CROSS REACTIVITY
    ...
    $cx[2] = array(53,34,118,119,231,43,252,39,18,19,49,20,140,27,32,4,5,40);
    //Sagebrush('F288','F031','F085',F382,'43','F208','F134','F351','F033','F013','F210','32','F129','F044','F329');
    ...
    //Ragweed('0024','0034','0144','0043','0039','0018','0019','0049','0020','0140','0027','0032','0004','0005','0264','0040',F344);
    $cx[4] = array(24,34,144,43,39,18,19,49,20,140,27,32,4,5,264,40,148);
    ...
    $cx[5] = array(473,131,132,133,134,135,34,58,59,43,62,51,69,20,137,138,139,140,27,32,4,71,141,5,75,104,40,77);
    //Latex('F049','F096','F092','F305','F031','F085','F103','F087','F153','F127','F035','F025','F329',F344);
