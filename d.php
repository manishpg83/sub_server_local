...
//  HEADER AND GLOBALS 
    ...
    if (strlen($client) == 6 AND strlen($patient) == 6){
      ...
      if (mysql_errno() == 1146){
       ...
       mysql_unbuffered_query("INSERT INTO $foodsTable SELECT * FROM `Foods999999` ORDER BY `alpha` ASC,`Description` ASC");
...
*/
    ...
    if ($tpl[6] == 1){  // FOOD FAMILIES 
      ...
      $results = @mysql_query($sql);
      while(list($id,$Family,$alpha,$Description,$Type) = mysql_fetch_array($results, MYSQL_NUM)){
        if ($alpha == 8){continue;}
        if ($savGrp != $alpha){
          $savGrp = $alpha;
          echo $pgBrk[$alpha]; 
