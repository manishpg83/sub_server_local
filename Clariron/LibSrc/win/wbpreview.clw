...
CvtLayoutColumnQueue QUEUE,TYPE
Cell                   &CvtLayoutCellClass
                     END

CvtLayoutRowQueue    QUEUE,TYPE
Columns                &CvtLayoutColumnQueue
                     END
...
Rows                   &CvtLayoutRowQueue,PRIVATE
...
Loc:Zoom             SHORT
Window WINDOW('Report Preview'),AT(,,394,283),FONT('Microsoft Sans Serif',8,,FONT:regular),SYSTEM,GRAY,RESIZE, |
         MDI
...
SetWindowTitle  ROUTINE
  ThisWindow.MyWindow{PROP:Text} = CLIP(sReportTitle)
  IF bShowPageNo
    ThisWindow.MyWindow{PROP:Text} = CLIP(ThisWindow.MyWindow{PROP:Text}) & ' - Page ' & POINTER(qReportPages)
  END
  IF bShowTotalPages
    ThisWindow.MyWindow{PROP:Text} = CLIP(ThisWindow.MyWindow{PROP:Text}) & ' of ' & RECORDS(qReportPages)
  END
  EXIT
...
Xpos            SIGNED,AUTO
  ...
  CODE
  Self.Rows.Columns &= NEW CvtLayoutColumnQueue
...
LayoutCvtHtmlClass.Init              PROCEDURE(STRING Style, SIGNED SnapX, SIGNED SnapY)
  ...
  SELF.Rows &= NEW CvtLayoutRowQueue
