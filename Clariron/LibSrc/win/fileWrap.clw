! $Header: /0192-7(XML support)/XMLSupport/src/fileWrap.clw 36    10/21/03 2:21p Mal $
!****************************************************************************
!  FILE..........: FileWrap.CLW
!  AUTHOR........: 
...
!
!------------------------------------------------------------------------------
FileWrapper.getCurrentValue           PROCEDURE!, string, PROTECTED,VIRTUAL 
blobIdx        UNSIGNED
memoIdx        UNSIGNED
sz             UNSIGNED
_              GROUP
...
Ref              &BLOB
        ...
        ELSE
            ...
            IF BAND(CLMEMO:BINARY,self.Fields.attr)
                RETURN clipNull(self.fileData{PROP:TEXT,-memoIdx})
            ELSE
                RETURN clip(self.fileData{PROP:TEXT,-memoIdx})
            END
        END                                        
!------------------------------------------------------------------------------
!
!------------------------------------------------------------------------------
FileWrapper.setCurrentValue           PROCEDURE(string val)!, PROTECTED,VIRTUAL
blobIdx        UNSIGNED
memoIdx        UNSIGNED
sz             UNSIGNED
_              GROUP
...
Ref              &BLOB
        ...
        ELSE
            !MEMO case
            self.fileData{PROP:TEXT,-memoIdx}=val
        END             
!------------------------------------------------------------------------------
!
!------------------------------------------------------------------------------
      ...
      if self.FileInfo.init(f) = XIErr:NoError then
        ...
        loop
          ...
          ELSE
            ...
            rc=self.ObjectInfo.getFieldInfo(blobMemos,memos,atr)
            self.Fields.blobMemoIdx=blobMemos
            self.Fields.attr=atr
            IF(sz=-1)
                !Blob case                
                self.Fields.memoIdx=0                                
            ELSE                
                self.Fields.memoIdx=memos                                
            END
          END 
