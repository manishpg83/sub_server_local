    ...
    if self.HR = S_OK
      if locRecordset_Count < 0   ! GetRecordCount is not supported
        ...
        if self.HR = S_OK
          ...
          lOrderByIndex = INSTRING(' ORDER BY', UPPER(SELF.Query), 1, 1)
          if ~lORderByIndex
            lOrderByIndex = LEN(SELF.Query)
          end
          szSize = len('SELECT ' & ' COUNT(*) as RecCount' & ' ' & SELF.Query[lFromIndex : lOrderByIndex]) + 1
          szCountStr &= new(cstring(szSize))
          szCountStr = 'SELECT ' & ' COUNT(*) as RecCount' & ' ' & SELF.Query[lFromIndex : lOrderByIndex]
