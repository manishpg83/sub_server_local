!  $Header: /0192-7(XML support)/XMLSupport/src/XMLSchema.clw 19    10/21/03 1:28p Mal $
!****************************************************************************
!  FILE..........: XMLSchema.clw
!  AUTHOR........: 
...
root        &Element,AUTO
            ...
            IF len(cs.str()) > 0 AND NOT size=-1
                !add size   
...
XMLSchema.open      PROCEDURE(STRING path, ULONG poolSize)!,BYTE
doc     &Document,auto
rc  BYTE
            CODE
            SELF.close   
            doc&=XMLFileToDOM(path, poolSize)
            IF(doc&=null)
...
cc  BYTE 
  ...
  END
  if(omitted(3))
    cc = DOMToXMLFile(doc, path,, Format,SELF.encoding)
  else
    cc = DOMToXMLFile(doc, path, newLine, Format,SELF.encoding)
  end
  IF(not cc)
    RETURN CPXMLErr:XMLWriteFail  
  END         
...
XMLSchema.setEncoding    procedure(XMLEnc enc)
            CODE
            SELF.encoding=enc
XMLSchema.getEncoding    procedure
            CODE
