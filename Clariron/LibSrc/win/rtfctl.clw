...
RTFControlClass.LinkPressed             PROCEDURE(STRING link)
 CODE

! =============================================================================

RTFControlClass.GetShowFullName         PROCEDURE()
 CODE
    RETURN SELF.ShowFullName

! =============================================================================

RTFControlClass.SetShowFullName         PROCEDURE(BYTE value)
 CODE
    SELF.ShowFullName = value

! =============================================================================

RTFControlClass.GetFileName             PROCEDURE()
 CODE
...
i         UNSIGNED,AUTO
  ...
  IF SELF.CtlShowFileName
     ...
     ELSE
        IF SELF.ShowFullName
           SELF.CtlShowFileName{PROP:Text} = SELF.FileName
        ELSE
           L = LEN (SELf.FileName)
           i = L
           LOOP
...
filename    CSTRING(2),AUTO
  CODE
  filename[1] = '<0>'
  SELF.Save (filename, mode)
  RETURN
...
W   WINDOW('Paragraph Indents'),AT(,,156,121),GRAY,AUTO,SYSTEM,FONT('Tahoma',8,,FONT:regular,CHARSET:ANSI),DOUBLE,center
        ...
        BUTTON('OK'),AT(46,97,45,14),USE(?OK),DEFAULT,TIP('Save changes')
        BUTTON('Cancel'),AT(96,97,45,14),USE(?Cancel),KEY(EscKey),TIP('Discard changes')
    END
...
W   WINDOW('Tabs'),AT(,,232,185),GRAY,IMM,AUTO,SYSTEM,FONT('Tahoma',8),ALRT(EscKey),DOUBLE,CENTER
        PROMPT('&Units:'),AT(10,12),USE(?Propmpt:Units),TRN
        ...
        END
        LIST,AT(25,47,106,116),USE(?TabList),FLAT,VSCROLL,RIGHT(2),TIP('List of ' & 'tab stops'),COLUMN,FROM(TabQ),FORMAT('20R(2)_@N-_6@')
        BUTTON('OK'),AT(163,10,45,14),USE(?OK),SKIP,DEFAULT,TIP('Save changes')
        BUTTON('Cancel'),AT(164,30,45,14),USE(?Cancel),SKIP,TIP('Discard changes')
        BUTTON('&Insert'),AT(164,66,45,14),USE(?Insert),KEY(InsertKey),SKIP, TIP('Add new tab stop')
        BUTTON('&Change'),AT(164,87,45,14),USE(?Change),KEY(CtrlEnter),SKIP,TIP('Change tab stop')
        BUTTON('&Delete'),AT(164,107,45,14),USE(?Delete),KEY(DeleteKey),SKIP,TIP('Delete tab stop')
        BUTTON('Clear &All'),AT(164,128,45,14),USE(?DoAll),KEY(CtrlDelete),SKIP,TIP('Clear all tab stops')
...
RTFControlClass.ContextMenuText         PROCEDURE ()
  CODE
     RETURN ''

! =============================================================================

RTFControlClass.ContextMenuChoice       PROCEDURE (LONG pSelectedChoice)
  CODE

! =============================================================================

RTFControlClass.ContextMenuTranslate    PROCEDURE (STRING pMenuText)
  CODE
     RETURN ''

! =============================================================================

RTFControlClass.AllowFormatChange       PROCEDURE ()
  CODE
  RETURN 0
...
RTFControlClass.RTFNotify.ContextMenuText PROCEDURE ()
  CODE
     RETURN SELF.ContextMenuText()

! =============================================================================

RTFControlClass.RTFNotify.ContextMenuChoice PROCEDURE (LONG opt)
  CODE
     SELF.ContextMenuChoice(opt)

! =============================================================================

RTFControlClass.RTFNotify.ContextMenuXlat  PROCEDURE (STRING pMenuText)
  CODE
     RETURN SELF.ContextMenuTranslate(pMenuText)

! =============================================================================

RTFControlClass.RTFNotify.LinkPressed  PROCEDURE (STRING link)
  CODE
     SELF.LinkPressed(link)
...
W   WINDOW('Find'),AT(,,236,62),GRAY,IMM,FONT('Tahoma',8),DOUBLE,CENTER
        PROMPT('Fi&nd what:'),AT(5,7),USE(?PROMPT1),RIGHT
...
W   WINDOW('Find and Replace'),AT(,,246,79),GRAY,IMM,SYSTEM,FONT('Tahoma',8),DOUBLE,CENTER
        PROMPT('Fi&nd what:'),AT(6,7),USE(?PROMPT1),RIGHT
        ENTRY(@s255),AT(51,7,131,12),USE(SText),LEFT(2)
        PROMPT('Replace with:'),AT(6,23),USE(?Prompt2),RIGHT
        ENTRY(@s255),AT(51,23,131,12),USE(RText),LEFT(2)
        CHECK(' &Whole words only'),AT(7,44),USE(WholeWord)
        OPTION('Direction'),AT(111,44,71,23),USE(Down),BOXED
            RADIO('&Up'),AT(119,55),USE(?ToTop),VALUE('0')
            RADIO('&Down'),AT(146,55),USE(?ToEnd),VALUE('1')
        END
        CHECK(' &Case sensitive search'),AT(7,57),USE(MatchCase)
        BUTTON('&Find Next'),AT(189,7,48,14),USE(?Find),DEFAULT
        BUTTON('&Replace'),AT(189,24,48,14),USE(?Replace)
        BUTTON('Replace &All'),AT(189,41,48,14),USE(?ReplaceAll)
        BUTTON('Cancel'),AT(189,58,48,14),USE(?Cancel)
    END
