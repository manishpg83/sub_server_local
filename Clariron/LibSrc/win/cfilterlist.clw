...
OpeStr &STRING
    ...
    loop pOpeGrp.Number times
      ...
      if self.ColOpeQ.Operator = '[COL] = [VAL]'
        self.DefaultOpeOnDrop = self.ColOpeQ.OperatorUser
      end
    end
...
IndicatorPos    long
    ...
    if sDropID
      ...
      ASCPos = instring(clip(SELF.ASCIndicator), clip(sDropID), 1, 1)
      DESCPos = instring(clip(SELF.DESCIndicator), clip(sDropID), 1, 1)
      if ASCPos or DESCPos
        if ASCPos
          IndicatorPos = instring(clip(SELF.ASCIndicator), clip(sDropID), 1, 1)
          sDropID = sDropID[1 : IndicatorPos - 1] & sDropID[IndicatorPos + len(clip(SELF.ASCIndicator)) : len(clip(sDropID))]
        end
        if DESCpos
          IndicatorPos = instring(clip(SELF.DESCIndicator), clip(sDropID), 1, 1)
          sDropID = sDropID[1 : IndicatorPos - 1] & sDropID[IndicatorPos + len(clip(SELF.DESCIndicator)) : len(clip(sDropID))]
        end
      end
      ...
      if Pos
        ...
        if ~errorcode()
          ...
          if SELF.DefaultOpeOnDrop
            SELF.FilterQ.Operator = SELF.DefaultOpeOnDrop
          else
...
Window WINDOW('Query Name'),AT(,,345,62),FONT('Microsoft Sans Serif',8,,FONT:regular,CHARSET:ANSI),CENTER,GRAY, |
         DOUBLE
       PROMPT('&Query Name:'),AT(15,14),USE(?Prompt1)
       ENTRY(@s50),AT(64,14,261,10),USE(sQuery)
       BUTTON('Ok'),AT(227,38,45,14),USE(?btnOk),DEFAULT
...
Window WINDOW('Query List'),AT(,,283,135),FONT('Microsoft Sans Serif',8,,FONT:regular,CHARSET:ANSI),CENTER,GRAY, |
         DOUBLE
       LIST,AT(8,8,266,95),USE(?List1),VSCROLL,ALRT(MouseLeft2),FORMAT('50L(2)|M~Query Name~@s50@'),FROM(PickQ)
       BUTTON('Select'),AT(130,108,45,14),USE(?btnSelect),DEFAULT
       BUTTON('Delete'),AT(180,108,45,14),USE(?btnDelete),KEY(DeleteKey)
