...
AskWindow WINDOW('Logon'),AT(,,160,77),FONT('Microsoft Sans Serif',8,,FONT:regular),CENTER,GRAY,NOFRAME
...
WbGuardClass.Init PROCEDURE(FileManager FileMgr, KEY FileKey, *? Name, *? Pswd, *? Capability, *? Id, <*? CntFailures>, <*? Locked>, <*? LockedUntil>, <ErrorClass ErrorHandler>)
  CODE
    ...
    ELSE
      SELF.ErrorStatus &= NEW ErrorStatusClass
      SELF.Errors &= NEW ErrorClass
      SELF.Errors.Init(SELF.ErrorStatus)
