...
!===  XMLString  ========
XMLStringToDOM procedure(string xml, ulong poolSize)

cs CStringClass

  code
  return CreateDocumentFromXML(cs.str(xml), poolSize)
...
cc                 byte
    ...
    return true
!===  XMLFILE  ========
XMLFileToDOM procedure(string path, ULONG poolSize)

cs CStringClass
doc &Document,auto
  code
  doc&=CreateDocumentFromFile(cs.str(path), poolSize)
  if(doc&=null)
...
FromXMLFile PROCEDURE(*file fl, string path,<string root>,<string label>,<MapQueue nameMap>,DOMStyle style=DOMStyle:ADO_26, ULONG poolSize)
fileDoc &Document, auto
cc      byte
map     &MapQueue
rt      &string
lbl     &string
            CODE
  fileDoc &= XMLFileToDOM(path, poolSize)!read data from the file
  if(fileDoc &= null)
    return false
  end
  if(not omitted(5))
...
FromXMLFile PROCEDURE(*queue que, string path,<string root>,<string label>,<MapQueue nameMap>,DOMStyle style=DOMStyle:ADO_26, ULONG poolSize)
fileDoc &Document, auto
cc      byte
map     &MapQueue
rt      &string
...
FromXMLFile PROCEDURE(*group gr, string path,<string root>,<string label>,<MapQueue nameMap>,DOMStyle style=DOMStyle:ADO_26, ULONG poolSize)
fileDoc &Document, auto
cc      byte
map     &MapQueue
rt      &string
...
Window WINDOW('XML'),AT(,,530,350),FONT('Microsoft Sans Serif',8,,FONT:regular),ICON(ICON:Application),SYSTEM, |
         GRAY,MAX,RESIZE
...
domW &DOMWriter, auto
  ...
  Window $ ?List{PROP:VLBProc} = address(MyWindow.VLBProc)
...
!This only translates single byte strings
ToUtf8 PROCEDURE(STRING in)
outStr &STRING, AUTO
i      SIGNED,AUTO
j      SIGNED(1)
  CODE
  outStr &= NEW STRING(SIZE(in) * 2)
  LOOP i = 1 TO SIZE(in)
    IF VAL(in[i]) <= 07FH
      outStr[j] = in[i]
      j += 1
    ELSE
      outStr[j] = CHR(0C0H + BSHIFT(VAL(in[i]), -6))
      j += 1
      outStr[j] = CHR(080H + BAND(03FH, VAL(in[i])))
      j += 1
    END
  END
  RETURN CLIP(outStr)

FromUtf8 PROCEDURE(STRING in)
outStr &STRING, AUTO
i      SIGNED,AUTO
j      SIGNED(1)
  CODE
  outStr &= NEW STRING(SIZE(in))
  LOOP i = 1 TO SIZE(in)
    IF VAL(in[i]) <= 07FH
      outStr[j] = in[i]
    ELSE
      outStr[j] = CHR(BSHIFT(BAND(VAL(in[i]), 3), 6) + BAND(VAL(in[i + 1]), 03FH))
      i += 1
    END
    j += 1
  END
