...
BaseArrayIterator     CLASS(CInterface2),TYPE      !,ABSTRACT
Cur                     LONG,PROTECTED
...
ArrayIIterator        CLASS(BaseArrayIterator),IMPLEMENTS(IIterator2),TYPE
Arr                     &ArrayI,PRIVATE
...
ArrayCIterator        CLASS(BaseArrayIterator),IMPLEMENTS(CIterator2),TYPE
Arr                     &Array,PRIVATE
...
Array._set              procedure(CInterface2 x, signed index)
cur                       long,auto
  code
  cur = address(x)
...
Array.doappend            procedure(CInterface2 next)
  code
  self._space()
  self._set(next, self.used-1)


Array.doadd               procedure(CInterface2 next, unsigned pos)
...
Array.replace           procedure(CInterface2 next, unsigned index, byte nodestruct)
cur                       &CInterface2,auto
  CODE
  if (NOT nodestruct)
    cur &= self.item(index)
...
Array.find              procedure(CInterface2 sought)
cur                     &CInterface2,auto
index                   signed(0)
  code
...
Array.zap               procedure(CInterface2 sought, BYTE nodel)
cur                       &CInterface2,auto
index                   signed(0)
  code
  loop while (index < self.used)
...
Array.zapall            procedure(CInterface2 sought, byte nodel)
cur                       &CInterface2,auto
ret                     byte(false)
index                   signed,auto
  code
...
? ASSERT(~Tmp &= NULL)
  Tmp.Init(SELF)
  RETURN Tmp.IIterator2


ArrayI.Kill PROCEDURE

cur   &IInterface2,AUTO
count SIGNED,AUTO
index SIGNED(0)
...
ArrayI._Set PROCEDURE(IInterface2 x, SIGNED index)
...
ArrayI.DoAppend PROCEDURE(IInterface2 next)
...
ArrayI.DoAdd PROCEDURE(IInterface2 next, UNSIGNED pos)
...
ArrayI.Replace PROCEDURE(IInterface2 next, UNSIGNED index, BYTE nodestruct)
...
ArrayI.Find PROCEDURE(IInterface2 sought)
...
ArrayI.zap PROCEDURE(IInterface2 sought, BYTE nodel)
...
ArrayI.ZapAll PROCEDURE(IInterface2 sought, byte nodel)
...
ArrayCIterator.CIterator2.Release PROCEDURE

  CODE
  SELF.Release


ArrayCIterator.CIterator2.Link PROCEDURE

  CODE
  SELF.Link


ArrayCIterator.CIterator2.First PROCEDURE

  CODE
  RETURN SELF.First()


ArrayCIterator.CIterator2.Next PROCEDURE

  CODE
  RETURN SELF.Next()


ArrayCIterator.CIterator2.IsValid PROCEDURE

  CODE
  RETURN SELF.IsValid()


ArrayCIterator.CIterator2.GetCurrent PROCEDURE

RVal   &CInterface2,AUTO
...
ArrayIIterator.IIterator2.Release PROCEDURE

  CODE
  SELF.Release                        ! Kill me...


ArrayIIterator.IIterator2.Link PROCEDURE

  CODE
  SELF.Link                           ! Increments usage count


ArrayIIterator.IIterator2.First PROCEDURE

  CODE
  RETURN SELF.First()


ArrayIIterator.IIterator2.Next PROCEDURE

  CODE
  RETURN SELF.Next()


ArrayIIterator.IIterator2.IsValid PROCEDURE

  CODE
  RETURN SELF.IsValid()


ArrayIIterator.IIterator2.GetCurrent PROCEDURE

RVal   &IInterface2,AUTO
