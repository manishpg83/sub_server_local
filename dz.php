...
.plus     {margin:0;display:inline-block;width:12px;height:12px;border:0;background-image: url('data:image/gif;base64,R0lGODlhDAAMAJH/AOQAE////wAxAGUAdywAAAAADAAMAAACFISPqQsRbBxsT7l7EcZLQs+A05gUADs=');}
.minus    {margin:0;display:inline-block;width:12px;height:12px;border:0;background-image: url('data:image/gif;base64,R0lGODlhDAAMAJH/AABuAP///wAxAGUAdywAAAAADAAMAAACDoSPqcvtG6KMaM6Hs8YFADs=');}
.checkmark{margin:0;display:inline-block;width:12px;height:12px;border:0;background-image: url('data:image/gif;base64,R0lGODlhDAAMAJEAAPZIAPv+/AAxAGUAdywAAAAADAAMAAACFISPqZsRDNxb8jh78atNTsVB4igWADs=');}
.up{margin:0;display:inline-block;width:12px;height:12px;border:1px solid #b80;background-image: url('data:image/gif;base64,R0lGODlhDAAMAJEAAPhSAv769wAxAGUAdywAAAAADAAMAAACF4SPqQsR3NxTUiYXs8GaI7+A1MSI0FEAADs=');}
.blank    {margin:0;display:inline-block;width:12px;height:12px;border:1px solid #000;background:#eee;}
.tested   {display:inline;padding:0 0 0 0;margin:0 0 0 .2em;font:700 .9em Arial,sans-serif;}
.strike   {display:inline;padding:0 0 0 0;margin:0 0 0 .2em;font:700 .9em Arial,sans-serif;text-decoration:line-through;}
.notTested{display:inline;padding:0 0 0 0;margin:0 0 0 .2em;font:400 .9em Arial,sans-serif;}
.hide{display:none;}
...
if (strlen($patient) == 6){
  ...
  if (strlen($error) == 0 AND $rows == 1){
	...
	if ($pat[5] > 0){
	  ...
	  $sql = "SELECT `Link` FROM `Patient` WHERE `Last` LIKE '$pat[1]' AND `First` LIKE '$pat[2]' ";
	  $results = @mysqli_query($dbc,$sql); 
	  $error = mysql_error();
	  $rows = @mysqli_num_rows($results);
	  $rows--;
...
$setInc = array(  
      0=> array(0=>0,1=>0,2=>0),
      1=> array(0=>0,1=>1,2=>1));
                                                                                                         //  $tdx++;$t[$tdx] = number_format((microtime(true) - $startTime),3);$last = $tdx-1;$et = $t[tdx] - $t[$last]; echo "<p>Start Data time: $t[$tdx]: $et</p>";
  $foods = array();
...
if ($tpl[5] == 1){   //  DIET  ##############################        
  $dietSubHdr[1] = "&#x2003;<div class=\"plus\"></div><p class=\"strike\">&#x2009;Positive Result&#x2003;</p><div class=\"minus\"></div><p class=\"tested\">&#x2002;Negative Result&#x2003;</p>
<div class=\"checkmark\"></div><p class=\"tested\">&#x2009;Positive Result, doctor included in diet&#x2003;</p>
...
$lineBrk[3] = array_fill(0,20,'</p></td><td>');
$lineBrk[3][4] = '</p></td></tr><tr><td>';
$lineBrk[3][9] = '</p></td></tr><tr><td>';
$lineBrk[3][14] = '</p></td></tr><tr><td>';
$lineBrk[3][19] = '</p></td><td></td></tr></table><hr class="divide"/>';
...
EOT;
  ...
  echo '<div class="col">';
  $sql = "SELECT `id`, `Family`,`alpha`,`Description`,`Day`,`Code`,`Group`,`Type` FROM $foodsTable WHERE $crossSQL[$ndx] = 1  ORDER BY $foodsTable.`Description` ASC";
  $results = @mysqli_query($dbc,$sql);
  $error = mysql_error();
  $SQL[$ndx] = $sql;
  while($row = mysqli_fetch_array($results, MYSQLI_NUM)){
    echo $icon[0][$foods[$row[0]][5]] . $foods[$row[0]][0] . "</p><br/>\n";
  }
  echo '</div></div>';
  $ndx++;
}while ($ndx < 5);
...
// echo '</div></div><div class="pgBrk"></div><br/>PATIENT<br/>SQL: '; 
// echo '<br><br><pre>FOODS<br/>' . var_export($foods,true)  . '</pre><br/>';
