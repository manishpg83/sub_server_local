...
<form action="https://amxemr.com/enter.php" method="post">
...
EOT;
  ...
  while (list($rec,$date,$last,$first,$dob,$gender) = @mysql_fetch_array($results, MYSQL_NUM)) {
    ...
    echo <<<EOT
<tr>
<td><form action="./eOffice/" method="post"><input type="hidden" name="rec" value="$rec" /><input type="hidden" name="sub" value="20" /><button class="ck1"  type="submit">$patient</button><input type="hidden" name="client" value="$id" /></form></td>
