...
$option = '';
$jsnpi = 'const npi = {'; 
$phy = "<div id=\"hcp\"><div class=\"label\">Healthcare Provider:</div> <input id=\"physican\" type=\"text\" name=\"physican\" value=\"$physician\"  autocomplete=\"off\"   />";

if($cnt > 0){
   $phy = "<div class=\"row\"><div class=\"label\">Healthcare Provider:</div> <input id=\"physican\" type=\"text\" name=\"physican\" value=\"$physician\"  list=\"phy\"placeholder=\"Double Click for List\" autocomplete=\"off\" onchange=\"getnpi();\"/><span class=\"red\"> &#x2731;</span></div><datalist id=\"phy\" >";
...
$phy .= "<p id=\"attest\"><input id=\"confirmation\" type=\"checkbox\" name=\"attest\" required onclick=\"confirm();\">
 I attest I am this provider, or authorized to order on their behalf.<span class=\"red\"> &#x2731;</span></p>";
...
var chk = document.getElementById('confirmation');
...
function getnpi(){
  let phy = document.getElementById('physican');
  console.log(phy);
  let val = phy.value ;
  let len = val.length;
...
function confirm(){
  ...
  if(len < 5){chk.checked = false;}
  document.getElementById('enablesub').style.display = enable[chk.checked];
}
window.onload = init;
