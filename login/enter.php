...
if($login
$orderbutton = '<form action="./" method="post"><input id="oo"$history type="submit" value="Create New Order, Online" /><input type="hidden" name="client" value="' . $client . '" /><input type="hidden" name="sub" value="40" /><input type="hidden" name="prior" value="6" /><input type="hidden" name="tab" value="5" /></form>';
echo <<<EOT
<!DOCTYPE html>
<html>
...
.v5{color:#ff0;}
.c5{color:#3bf;}
.h1,.h2,.h3,.h4,.h5,.h6,#sub,#lo,#ss,#ph,#clear,#announce,#close,#fh,#oo{font-weight:700;border:0;padding:0;margin:0;color:#fff;background:#66CCCC;
background-image: linear-gradient(to bottom, #164 0%, #033 100%);}
#sub,#lo,#ss,#ph,#clear,.box,#announce,#close,#fh,#oo{border:2px ridge #6cc;margin-top:4px;}
...
h2{margin-bottom:0;}
#sub,#lo,#ss,#ph,#clear,#announce,#close,#fh,#oo{width:100%;font:700 1em Arial,sans-serif;padding:.25em;}
...
<form action="requestForm.php"><input type="hidden" name="id" value="$id" />
<input id="fh" $history type="submit" value="Patient Inhalant &amp; Food History with Request Form" /></form>
$orderbutton
</div>
