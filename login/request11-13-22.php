...
$sql = "UPDATE `orders` SET `status`= 'O' WHERE `id` = $rec";
...
.msg{font:400 1em Arial;margin:0 0 10px 89px;color:#f00;}
...
#bdi{color:#c11;font-weight:700;}
.hide{display:none;}
textarea{width:100%;margin-bottom:10px;}
</style></head><body><div id="post">
<div id="header">
<img src="allermetrix_E-office600.jpg" alt="Allermetrix E-Office" /><br>
$clientname Patient:$first $last Order: $rec 
</div>
EOT;
...
$option = '';
$jsnpi = 'const npi = {'; 
   $phy = "<div class=\"row\"><div class=\"label\">Healthcare Provider:</div> <input id=\"physican\" type=\"text\" name=\"physican\" value=\"$physician\"  autocomplete=\"off\"   />";

if($cnt > 0){
   $phy = "<div class=\"row\"><div class=\"label\">Healthcare Provider:</div> <input id=\"physican\" type=\"text\" name=\"physican\" value=\"$physician\"  list=\"phy\"placeholder=\"Double Click for List\" autocomplete=\"off\" oninput=\"getnpi()\";/></div><datalist id=\"phy\" >";
   while(list($code,$physician,$npi) = mysqli_fetch_array($results, MYSQLI_NUM)){
     $phy .= " <option name=\"$npi\" value=\"$physician\">$physician</option>\n";
     $jsnpi .= "'$physician':'$npi',";
  }
  $phy .= "</datalist>\n"; 
}
...
if(strlen($address) < 5 || strlen($zip) < 5 || strlen($state) < 2 || strlen($city) < 3 ){
  $addrequest[4] = '<div id="address">Patient Address is essential for Patient Pay.  If you have it available now, please enter it now.</div>';
  $addrequest[5] = '<div id="address">Patient Address is essential for Insurance.  If you have it available now, please enter it now.</div>';
  $addrequest[6] = '<div id="address">Patient Address is essential for Patient Pay.  If you have it available now, please enter it now.</div>';
}
$icdrequest = array('','','','','','','');
if(strlen($icd) < 3){
...
<div id="patient">&#x2003;
<div class="msg"><span class="red"> &#x2731;</span> Required</div>
$clientName

<div class="row"><div class="plabel">Last:</div> <input class="patient" type="text" name="last" value="$last"  required/><span class="red"> &#x2731;</span></div>
<div class="row"><div class="plabel">First:</div> <input class="patient" type="text" name="first" value="$first" required/><span class="red"> &#x2731;</span></div>
...
<div>
<button type="button" id="show" onclick="exp('7')">Show Order Details</button>
</div>
<div id="i7">
EOT;
ob_flush();
...
<p>$first $last <br>
Order Number: $rec<br>
Billing: $billingType[$billing]<br>
Needed: $needed mL</p>
...
<input class="submit" type="submit" value="Submit to Allermetrix and Print" />
...
function exp(id){
  ...
  prev=id;
  show.innerHTML = text[buttons[prev][0].style.display];
}
function billingChange(){
  document.getElementById('sub').value = 58;
  document.forms["frm"].submit();
}
function getnpi(){
  let phy = document.getElementById('physican').value ;
