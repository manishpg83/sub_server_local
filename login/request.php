...
$initials = substr($_POST['initials'],1,4);
...
.text{padding:2px 6px;marginline-bottom:2px;font:400 1em/1 Arial,sans-serif;}
.text1{padding:2px 6px;marginline-bottom:2px;font:400 1em Arial,sans-serif;}
...
#hcp{width:500px;}
.hide{display:none;}
textarea{width:100%;marginline-bottom:10px;}
#provider{display:none;border-style: outset;margin:10px 0 10px 0;}
#initials{margin:5px 0 10px 0;}
yourInitials{none;}
input::placeholder {
    font-weight: bold;
    opacity: .6;
...
#asteriskFirst,#asteriskLast,#asteriskDOB,#asteriskGender{display:none;color:#f00;}
</style></head><body>
...
$option = '';
$jsnpi = 'const npi = {'; 
$phy = "<div id=\"hcp\"><div class=\"label\">Healthcare Provider:</div> <input id=\"physican\" type=\"text\" name=\"physican\" value=\"$physician\"  autocomplete=\"off\"  oninput=\"getlength();\" /><span id=\"asteriskPhy\" class=\"required\"> &#x2731;&#x2731;</span>";

if($cnt > 0){
   $phy = "<div id=\"hcp\"><div class=\"label\">Healthcare Provider:</div> <input id=\"physican\" type=\"text\" name=\"physican\" value=\"$physician\"  list=\"phy\"placeholder=\"Click for List or Type Name\" autocomplete=\"off\" oninput=\"getnpi();\"/><span id=\"asteriskPhy\" class=\"required\"> &#x2731;&#x2731;</span></div><datalist id=\"phy\" >";
...
<p id=\"attest\"><input id=\"confirmation\" type=\"radio\" name=\"attest\" value=\"1\" required  onclick=\"confirm(1);\" >
 ...
 <p id=\"yourInitials\">&emsp;Your Initials <input id=\"initials\" type=\"text\" name=\"initials\" oninput=\"chkYourInitials()\"></p>
...
<div class="row"><div class="plabel">Last:</div> <input id="last" class="patient" type="text" name="last" value="$last"  required  onchange="chkAll()"/><span id="asteriskLast"> &#x2731;&#x2731;</span></div>
<div class="row"><div class="plabel">First:</div> <input id="first" class="patient" type="text" name="first" value="$first" required  onchange="chkAll()"/><span id="asteriskFirst"> &#x2731;&#x2731;</span></div>
<div class="row"><div class="plabel">DoB:</div> <input id="dob" type="date" name="dob" value="$dob" onchange="chkAll()"/><span id="asteriskDOB" class="required"> &#x2731;</span></div>
<div>Gender:
<div class="radio"><input id="male" type="radio"  name="gender" value="1" $checked[1] required onchange = chkMF();  />&thinsp;Male&#x2002;</div>
<div class="radio"><input id="female" type="radio"  name="gender" value="2" $checked[2]  onchange = chkMF(); />&thinsp;Female</div><span id="asteriskGender"> &#x2731;&#x2731;</span></div>
...
$needed = $_POST['needed'];
$bdi = '';
if($Accession < 100000){$bdi = '<input type="checkbox" name="bdi" value="1" ><span id="bdi">&ensp;Includes patient information / blood drawing instructions and patient responsibility form (if required)</span><br>';}
echo <<<EOT
...
var asteriskPhy = document.getElementById('asteriskPhy');
var asteriskDOB = document.getElementById('asteriskDOB');
var asterisk = document.getElementById('asteriskGender');
...
function init(){
  ...
  if(last.value.length == 0){asteriskLast.style.display = 'inline-block';}
  if(first.value.length == 0){asteriskFirst.style.display = 'inline-block';}
  
  asteriskDOB.style.display = 'none';
  let age = Math.round((new Date() - (new Date(dob.value)))/31536000000);
  if (age < 1 || age > 100) {
    asteriskDOB.style.display = 'inline-block';
  }
...
function getnpi(){
  ...
  if(npi[val] == undefined){
    ...
    if (len > 2){
      ...
      asteriskPhy.style.display = 'none';
    }
    else{
      if(len == 0){last.focus();phy.focus();}
      asteriskPhy.style.display = 'inline-block';
      provider.style.display = 'none';
      attest[0].checked = false;
      attest[1].checked = false;
      attest[2].checked = false;
  ...
  console.log('getnpi phy val: ' + val + ',' + len  + '|provider: ' + provider.style.display  + '|sub: ' + enablesub.style.display );
  chkAll();
}
...
function confirm(num){
  if(num == 1 || num == 3 ){
    ...
    yourInitials.style.display = 'inline-block';
    initials.focus();
    enablesub.style.display = 'none';
  }
  console.log('confirm: ' + num + '|phy:' + phy.value.length + '|sub: ' + enablesub.style.display);
}

function getlength(num){
    
    
    if(phy.value.length > 2){
      provider.style.display = 'block';
    }
    if(phy.value.length < 3){
      provider.style.display = 'none';
      yourInitials.style.display = 'none';
      initials.value = '';
      attest[0].checked = false;
	...
	console.log('getlength phy: ' + phy.value.length + '|provider: ' + provider.style.display + '|sub: ' + enablesub.style.display );
    chkAll();
}
function chkMF(){
  //initials.value = '';
...
function chkAll(){
  missing = 0b0;
  let len = initials.value.length;
  let showRequired = 'none';
  enablesub.style.display = 'none';
  if(len > 1){enablesub.style.display = 'block';}
  asteriskLast.style.display = 'none';
  if(last.value.length < 2){asteriskLast.style.display = 'inline-block';showRequired = 'inline-block';missing += 0b10;}
  asteriskFirst.style.display = 'none';
  if(first.value.length < 1){asteriskFirst.style.display = 'inline-block';showRequired = 'inline-block';missing += 0b100;}
  let age = Math.round((new Date() - (new Date(dob.value)))/31536000000);
  asteriskDOB.style.display = 'none';
  if (age < 1 || age > 100 || isNaN(age)) {
    asteriskDOB.style.display = 'inline-block';
    showRequired = 'inline-block';
    missing += 0b1000;
  }
  asteriskGender.style.display = 'inline-block';
  missing += 0b10000;
  if(male.checked){asteriskGender.style.display = 'none';missing -= 0b10000;}
  if(female.checked){asteriskGender.style.display = 'none';missing -= 0b10000;}
  required.style.display = showRequired;
  chkMissing();
  val = phy.value;
  let trimed = val.trim();
  len = trimed.length;
//  provider.style.display = 'none';
  ...
  console.log('chkAll: initials:' + initials.value + '|first: ' + first.value.length + '| last: ' + last.value.length +  '|age: ' + age + '|provider: ' + provider.style.display  + '|sub: ' + enablesub.style.display + '|missing: ' + missing);

}
function chkMissing(){ 
  if(missing > 15){enablesub.style.display = 'none';hcp.style.display = 'none';return; }
  hcp.style.display = 'block';
  if((missing & 6) > 0){physican.value = '';initials.value = '';attest[0].checked = false;attest[1].checked = false;attest[2].checked = false;hcp.style.display = 'none';}
  if(missing & 0b10){last.focus();let len = last.value.length;last.setSelectionRange(len, len);return;}
  if(missing & 0b100){first.focus();let len = first.value.length;first.setSelectionRange(len, len);return;}

}

function chkYourInitials(){
  if(missing > 15){enablesub.style.display = 'none';return;}
  ...
  console.log('miss:' + missing + ' initials: ' + initials.value + '|sub: ' + enablesub.style.display );
}
window.onload = init;
